<template>
  <div class="linx-panel p-5">
    ...
    <div class="space-y-3">
      <button @click="router.push('/getting-started')" class="btn btn-secondary group flex w-full items-center gap-4 p-4 text-left">
        <div class="flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-xl bg-primary-500/10 transition-transform group-hover:scale-105">
          <Icon name="book" size="lg" class="text-primary-500 dark:text-primary-400" />
        </div>
        <div class="min-w-0 flex-1">
          <p class="text-sm font-medium text-gray-900 dark:text-white">{{ t('gettingStarted.dashboard.quickActionTitle') }}</p>
          <p class="text-xs text-gray-500 dark:text-linear-ink-subtle">{{ t('gettingStarted.dashboard.quickActionDescription') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
      ...
      <button @click="router.push('/keys')" class="btn btn-secondary group flex w-full items-center gap-4 p-4 text-left">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xs text-gray-500 dark:text-linear-ink-subtle">{{ t('dashboard.generateNewKey') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
      ...
      <button @click="router.push('/usage')" class="btn btn-secondary group flex w-full items-center gap-4 p-4 text-left">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xs text-gray-500 dark:text-linear-ink-subtle">{{ t('dashboard.checkDetailedLogs') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
      ...
      <button v-if="canUseBatchImage" @click="router.push('/batch-image')" class="btn btn-secondary group flex w-full items-center gap-4 p-4 text-left">
      ...
      <button @click="router.push('/redeem')" class="btn btn-secondary group flex w-full items-center gap-4 p-4 text-left">
        ...
        <div class="min-w-0 flex-1">
          ...
          <p class="text-xs text-gray-500 dark:text-linear-ink-subtle">{{ t('dashboard.addBalanceWithCode') }}</p>
        </div>
        <Icon
          name="chevronRight"
          size="md"
