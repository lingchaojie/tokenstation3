...
describe('UserPlatformQuotaCell', () => {
  ...
  it('多平台按 anthropic→openai→kilo→gemini→antigravity 顺序，仅展示有限额的', () => {
    const w = mount(UserPlatformQuotaCell, {
      props: {
        quotas: [
          item({ platform: 'gemini', monthly_limit_usd: 50 }),
          item({ platform: 'kilo', daily_limit_usd: 20 }),
    ...
    const text = w.text()
    expect(text.indexOf('anthropic')).toBeLessThan(text.indexOf('kilo'))
    expect(text.indexOf('kilo')).toBeLessThan(text.indexOf('gemini'))
