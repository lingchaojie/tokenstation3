  ...
  return mount(AvailableChannelsTable, {
    ...
    global: {
      ...
      stubs: {
        ...
        GroupBadge: {
          props: ['name', 'showRate'],
          template: '<span data-group-badge>{{ name }}:{{ showRate }}</span>',
        },
        SupportedModelChip: {
...
describe('AvailableChannelsTable responsive surfaces', () => {
  ...
  it('renders a mobile-only readable surface without exposing internal group rates', () => {
    const wrapper = mountTable()
    ...
    expect(mobile.get('[data-group-badge]').text()).toBe('Exclusive Pro:false')
    expect(mobile.findAll('[data-group-badge]')).toHaveLength(2)
    expect(mobile.find('[data-icon="clock"]').exists()).toBe(false)
    expect(componentSource).not.toContain('rate_multiplier')
    expect(componentSource).not.toContain('userGroupRates')
