...
import { defaultReasoningEffortMultiplier, formatScaled, resolveIntervalPrices } from '@/utils/pricing'
...
function reasoningEffortMultipliers(model: PlazaModel): [string, number][] {
  ...
  return REASONING_EFFORT_LEVELS.flatMap(effort => {
    const fallback = defaultReasoningEffortMultiplier(model.name, effort)
    const multiplier = multipliers?.[effort] ?? (fallback !== 1 ? fallback : undefined)
