<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="account && isUpstreamBillingProbeCapable(account.platform, account.type)"
        class="flex items-center justify-between gap-4 border-t border-gray-200 pt-4 dark:border-dark-600"
      >
        <div>
      ...
      <div class="border-t border-gray-200 pt-4 dark:border-dark-600">
        ...
        <div v-if="account?.platform === 'antigravity' || account?.platform === 'kiro'" class="flex items-center gap-2">
          <label class="flex cursor-pointer items-center gap-2">
            <input
              ...
              :class="account?.platform === 'antigravity' ? 'cursor-not-allowed opacity-60' : ''"
        ...
        <div v-if="account?.platform === 'kiro' && mixedScheduling" class="mt-3 space-y-3 rounded-md border border-blue-200 bg-blue-50 p-3 dark:border-blue-700 dark:bg-blue-900/20">
...
const loadOpenCodeGoUsage = async () => {
  const generation = opencodeGoGeneration
  opencodeGoLoading.value = true
  try {
    const state = await adminAPI.accounts.getOpenCodeGoUsage(props.account!.id)
    if (generation !== opencodeGoGeneration) return
    applyOpenCodeGoState(state)
  } catch (error) {
    if (generation !== opencodeGoGeneration) return
    appStore.showError(extractApiErrorMessage(error, t('admin.accounts.opencodeGo.loadFailed')))
  } finally {
    if (generation === opencodeGoGeneration) opencodeGoLoading.value = false
  }
}
...
const setOpenCodeGoAutoRefresh = async (enabled: boolean) => {
  ...
  try {
    const state = await adminAPI.accounts.setOpenCodeGoUsageAutoRefresh(props.account!.id, enabled)
    if (generation !== opencodeGoGeneration) return
    applyOpenCodeGoState(state)
  } catch (error) {
    if (generation !== opencodeGoGeneration) return
    ...
    if (generation === opencodeGoGeneration) opencodeGoSaving.value = false
  }
}
...
const refreshOpenCodeGoUsage = async () => {
  ...
  try {
    const state = await adminAPI.accounts.refreshOpenCodeGoUsage(props.account!.id)
    if (generation !== opencodeGoGeneration) return
    applyOpenCodeGoState(state)
    ...
    if (generation === opencodeGoGeneration) opencodeGoRefreshing.value = false
  }
}

watch(() => [props.show, props.account?.id] as const, ([show], _, onCleanup) => {
  onCleanup(() => { opencodeGoGeneration++ })
  ...
  if (show && opencodeGoState.value?.eligible && !opencodeGoState.value.snapshot) void loadOpenCodeGoUsage()
}, { immediate: true })
...
const syncFormFromAccount = (newAccount: Account | null) => {
  ...
  if (newAccount.type === 'apikey' && newAccount.credentials) {
    ...
    // Load model mappings for OAuth accounts with platform-specific credential semantics.
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (isUpstreamBillingProbeCapable(props.account.platform, props.account.type)) {
    ...
    if (props.account.platform === 'kiro' && !isKiroRelay.value) {
      ...
      if (mixedScheduling.value) {
        ...
        delete newExtra.kiro_auto_sticky_enabled
        delete newExtra.kiro_sticky_session_ttl_seconds
      }
