...
describe('isUpstreamBillingProbeCapable', () => {
  it('matches the backend API-key platform whitelist exactly', () => {
    for (const platform of ['openai', 'anthropic', 'gemini', 'antigravity', 'grok']) {
      expect(isUpstreamBillingProbeCapable(platform, 'apikey')).toBe(true)
    }

    expect(isUpstreamBillingProbeCapable('kiro', 'apikey')).toBe(false)
    expect(isUpstreamBillingProbeCapable('openai', 'oauth')).toBe(false)
    expect(isUpstreamBillingProbeCapable('antigravity', 'upstream')).toBe(false)
  })
})
