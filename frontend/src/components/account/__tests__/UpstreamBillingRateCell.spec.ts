...
describe('UpstreamBillingRateCell', () => {
  ...
  it('shows manual probe controls only for the backend API-key platform whitelist', async () => {
    const wrapper = mount(UpstreamBillingRateCell, {
    ...
    for (const platform of ['anthropic', 'gemini', 'antigravity', 'grok'] as const) {
    ...
    await wrapper.setProps({ account: makeAccount({ platform: 'kiro' }) })
    expect(wrapper.find('[data-testid="upstream-billing-probe"]').exists()).toBe(false)
    expect(wrapper.text()).toBe('-')

    await wrapper.setProps({ account: makeAccount({ platform: 'openai', type: 'oauth' }) })
