...
describe('OpenCodeGoUsageCell', () => {
  ...
  it('ignores a refresh from a previous account after the row is reused', async () => {
    let complete!: (state: OpenCodeGoUsageState) => void
    refreshOpenCodeGoUsage.mockReturnValueOnce(new Promise(resolve => { complete = resolve }))
    const wrapper = mount(OpenCodeGoUsageCell, { props: { account: account() } })
    await wrapper.get('[data-testid="opencode-go-usage-query"]').trigger('click')
    const next = usageState({ account_id: 8 })
    next.snapshot!.data!.rolling!.percent = 17
    await wrapper.setProps({ account: { ...account(next), id: 8 } })
    complete(usageState())
    await flushPromises()
    expect(wrapper.findAllComponents(UsageProgressBar)[0].props('utilization')).toBe(17)
