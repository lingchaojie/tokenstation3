...
/** API channel pricing → editable form pricing without dropping billing fields. */
export function apiChannelPricingToForm(pricing: ChannelModelPricing): PricingFormEntry {
  return {
...
export function getPlatformTagClass(platform: string): string {
  switch (platform) {
    ...
    case 'cursor': return 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/30 dark:text-cyan-300'
...
export function getPlatformTextClass(platform: string): string {
  switch (platform) {
    ...
    case 'cursor': return 'text-cyan-700 dark:text-cyan-300'
    default: return ''
  }
}
