...
vi.mock('vue-i18n', async importOriginal => ({
  ...
  useI18n: () => ({ t: (key: string, params?: Record<string, unknown>) =>
    key === 'admin.channels.form.reasoningEffortMultiplierDefault' ? `Default: ${params?.multiplier}` : key }),
}))
...
describe('PricingEntryCard request multipliers', () => {
  ...
  it('shows the Fable max default without storing it as an explicit override', () => {
    const wrapper = shallowMount(PricingEntryCard, {
    ...
    expect(wrapper.get('[data-reasoning-effort="max"]').attributes('placeholder'))
      .toBe('Default: 3')
    expect(wrapper.get('[data-reasoning-effort="high"]').attributes('placeholder')).toBe('Default: 1')
  })
