  ...
  it('保存提交完整 9 platform payload，并保留国产平台现有值', async () => {
    apiMocks.getPlatformQuotas.mockResolvedValueOnce({
      platform_quotas: [
        ...
        { platform: 'kimi', daily_limit_usd: 3, weekly_limit_usd: 12, monthly_limit_usd: 40,
          daily_usage_usd: 1, weekly_usage_usd: 2, monthly_usage_usd: 3 },
        { platform: 'zhipu', daily_limit_usd: 4, weekly_limit_usd: 16, monthly_limit_usd: 50,
          daily_usage_usd: 1, weekly_usage_usd: 2, monthly_usage_usd: 3 },
        { platform: 'deepseek', daily_limit_usd: 5, weekly_limit_usd: 20, monthly_limit_usd: 60,
          daily_usage_usd: 1, weekly_usage_usd: 2, monthly_usage_usd: 3 },
      ],
    })
    const w = await mountAndOpen()
    ...
    expect(payload.find((p: any) => p.platform === 'kimi')).toMatchObject({
      daily_limit_usd: 3, weekly_limit_usd: 12, monthly_limit_usd: 40,
    })
    expect(payload.find((p: any) => p.platform === 'zhipu')).toMatchObject({
      daily_limit_usd: 4, weekly_limit_usd: 16, monthly_limit_usd: 50,
    })
