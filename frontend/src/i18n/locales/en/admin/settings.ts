    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          firstRechargeThresholdHint: 'Set 0 to grant at registration. Above 0, only an invitee’s qualifying first recharge grants rewards (subscriptions always qualify).',
          immediateModeHint: 'Immediate mode: both rewards are granted as soon as the new user registers with the invite code.',
          firstRechargeModeHint: 'First-recharge mode: both rewards are granted only when the first recharge qualifies.',
          inviterReward: 'Inviter Reward',
          inviterRewardHint: 'Fixed Token credit granted to the inviter when the configured condition is met.',
          inviteeReward: 'Invitee Reward',
          inviteeRewardHint: 'Fixed Token credit granted to the invitee when the configured condition is met.',
          rewardValidityDays: 'Reward Validity (days)',
          rewardValidityDaysHint: 'How long each inviter and invitee reward remains valid after it is granted.',
          inviterRewardLimit: 'Inviter Reward Count Limit',
          inviterRewardLimitHint: '0 means unlimited. At the cap, the inviter stops earning but new invitees still receive their reward.',
          title: 'Affiliate (Invite Rebate)',
          description: 'Configure immediate or first-recharge rewards, both amounts, expiry, and the inviter reward-count cap. Disabled by default.',
          enabled: 'Enable Affiliate',
          enabledHint: 'When off, affiliate entry points and signup codes are disabled, and no new fixed rewards or percentage rebates are granted. Existing reward credits keep their original expiry, and existing rebate balances can still be transferred.',
