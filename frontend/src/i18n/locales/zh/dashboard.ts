export default {
  ...
  keys: {
    ...
    useKeyModal: {
      unified: {
        description: '这是一个统一密钥——同时适用于 Anthropic（Claude Code）和 OpenAI（Codex）客户端。请选择你所使用工具对应的标签页，转发服务会自动路由每个请求。',
      ...
      grok: {
        description:
          '配置 Grok CLI 或 OpenCode，让请求通过当前 Sub2API Grok 分组发送。文本模型走 Responses；图片/视频使用 Imagine 模型 ID 与媒体端点。',
        configTomlHint:
  ...
  modelPlaza: {
    ...
    table: {
      ...
      reasoningMultiplierHint: '最终转发的思考等级为 {effort} 时，整次请求的计费与额度消耗乘以 {multiplier}；未配置时 Fable 5.1 的 max 按 3 倍，其他等级按 1 倍计费',
