    ...
    settings: {
      ...
      features: {
        ...
        affiliate: {
          ...
          firstRechargeThresholdHint: '0 表示注册后立即发放；大于 0 时，被邀请用户首次充值到账额达到该金额后才发放（订阅无条件达标）。',
          immediateModeHint: '当前为立即奖励模式：新用户注册并绑定邀请码后，双方奖励立即到账。',
          firstRechargeModeHint: '当前为首充奖励模式：仅首笔充值达标时发放双方奖励。',
          inviterReward: '邀请方奖励',
          inviterRewardHint: '满足当前奖励条件后，邀请方获得的固定 Token 额度。',
          inviteeReward: '被邀请方奖励',
          inviteeRewardHint: '满足当前奖励条件后，被邀请方获得的固定 Token 额度。',
          rewardValidityDays: '奖励有效期（天）',
          rewardValidityDaysHint: '每笔邀请方和受邀方奖励从到账时起计算的有效天数。',
          inviterRewardLimit: '每人邀请方获奖次数上限',
          inviterRewardLimitHint: '0 表示不限制。达到上限后邀请方不再获奖，但新受邀用户仍可获得受邀方奖励。',
          title: '邀请返利',
          description: '支持立即奖励或首充达标奖励；双方金额、有效期和邀请方获奖次数上限均可配置。默认关闭。',
          enabled: '启用邀请返利',
          enabledHint: '关闭后用户菜单中的邀请页面入口隐藏、注册页不接收邀请码、邀请链接回到普通注册，新充值不再产生返利。已有奖励余额仍按原到期时间使用。',
          rebateRate: '全局返利比例',
