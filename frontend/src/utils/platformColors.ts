...
export type Platform = 'anthropic' | 'openai' | 'antigravity' | 'gemini' | 'kiro' | 'grok' | 'cursor'
...
const BORDER_STRONG: Record<Platform, string> = {
  ...
  cursor: 'border-cyan-500/35 dark:border-cyan-500/30',
}
...
const ACCENT_BAR: Record<Platform, string> = {
  ...
  cursor: 'bg-gradient-to-r from-cyan-400 to-cyan-500',
}
...
const ICON: Record<Platform, string> = {
  ...
  grok: 'text-zinc-800 dark:text-zinc-200',
  cursor: 'text-cyan-600 dark:text-cyan-300',
}
...
const BUTTON: Record<Platform, string> = {
  ...
  cursor: 'bg-cyan-600 text-white hover:bg-cyan-700 active:bg-cyan-800 dark:bg-cyan-600/80 dark:hover:bg-cyan-600',
}
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'antigravity' || p === 'gemini' || p === 'kiro' || p === 'grok' || p === 'cursor'
}
