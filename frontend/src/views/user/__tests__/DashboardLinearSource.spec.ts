...
const dashboardSource = readFileSync(resolve(userDir, 'DashboardView.vue'), 'utf8')
...
describe('Dashboard Linear page contract', () => {
  it('wraps dashboard content in Linear page and panel classes', () => {
    expect(dashboardSource).toContain('linear-dashboard-page')
    expect(dashboardSource).toContain('linx-panel')
    expect(dashboardSource).toContain('lg:grid-cols-[1.2fr_0.8fr]')
  })
  ...
  it('keeps nested chart panels stacked until very wide screens', () => {
    expect(dashboardSource).toContain('lg:grid-cols-[1.2fr_0.8fr]')
    expect(dashboardSource).toContain('min-w-0 space-y-5')
