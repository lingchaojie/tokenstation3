...
describe("groupsReasoningEffort", () => {
  it("provides fixed choices to OpenAI groups", () => {
    const expected = [
      "minimal",
      "low",
      "medium",
    ...
    for (const platform of ["openai"] as const) {
      expect(
    ...
    expect(
      reasoningEffortSourceOptionsForPlatform("anthropic").map((option) => option.value),
  ...
  it("hydrates and validates none only as a mapping source", () => {
    ...
    expect(normalizeReasoningEffortSourceForPlatform("anthropic", " NONE ")).toBe("none");
