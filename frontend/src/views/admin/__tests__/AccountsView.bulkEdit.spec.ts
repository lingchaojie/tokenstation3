...
describe('admin AccountsView bulk edit scope', () => {
  ...
  it('uses exact metadata from selected accounts across pages for bulk edit eligibility', async () => {
    const account = (id: number, platform: 'openai' | 'kiro') => ({
      id,
      name: `account-${id}`,
      platform,
      type: 'apikey',
    ...
    listAccounts
      .mockResolvedValueOnce({ items: [account(7, 'openai')], total: 2, page: 1, page_size: 1, pages: 2 })
      .mockResolvedValueOnce({ items: [account(11, 'kiro')], total: 2, page: 2, page_size: 1, pages: 2 })
    ...
    expect(modal.attributes('data-target-platforms')).toBe('openai,kiro')
    expect(modal.attributes('data-target-types')).toBe('apikey')
  })

  it('loads exact metadata beyond the first 100 filtered accounts before opening bulk edit', async () => {
    const accounts = Array.from({ length: 101 }, (_, index) => ({
      id: index + 1,
      name: `account-${index + 1}`,
      platform: index === 100 ? 'kiro' : 'openai',
      type: 'apikey',
      status: 'active',
      schedulable: true,
      created_at: '2026-07-13T00:00:00Z',
    ...
    expect(modal.attributes('data-target-platforms')).toBe('openai,kiro')
    expect(modal.attributes('data-target-types')).toBe('apikey')
    expect(modal.attributes('data-preview-count')).toBe('101')
  })
  ...
  it('submits only eligible selected account IDs from every page', async () => {
    const account = (id: number, platform: 'openai' | 'kiro' = 'openai') => ({
      id,
      name: `account-${id}`,
      platform,
      type: 'apikey',
  ...
  it('keeps the configured account rate after a batch probe returns a legacy writeback hint', async () => {
    ...
    expect(listAccounts).toHaveBeenCalledTimes(2)
    expect(wrapper.get('[data-test="account-rate"]').text()).toBe('0.25x')
  })
  ...
  it('keeps the configured account rate after a single probe returns a legacy writeback hint', async () => {
