...
describe('HomeView landing page', () => {
  ...
  it('keeps the built-in API docs entry separate from configured external docs', async () => {
    ...
    expect(apiDocsLink.element.parentElement?.className).toContain('hidden')
    expect(apiDocsLink.element.parentElement?.className).toContain('md:flex')

    const mobileApiDocsLink = wrapper.get('[data-testid="api-docs-mobile-nav-link"]')
    expect(mobileApiDocsLink.attributes('href')).toBe('/docs')
    expect(mobileApiDocsLink.attributes('aria-label')).toBe('API 文档')
    expect(mobileApiDocsLink.classes()).toContain('md:hidden')
    expect(mobileApiDocsLink.text()).toBe('API 文档')
  ...
  it('renders URL custom home content in a full-page iframe before the default landing page', async () => {
    ...
    expect(wrapper.find('[data-testid="api-docs-mobile-nav-link"]').exists()).toBe(false)
