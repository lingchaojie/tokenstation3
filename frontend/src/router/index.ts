...
const routes: RouteRecordRaw[] = [
  ...
  {
    path: '/admin/my-account/dashboard',
    name: 'AdminMyAccountDashboard',
    component: () => import('@/views/admin/MyAccountDashboardView.vue'),
    meta: {
      requiresAuth: true,
      requiresAdmin: true,
      title: 'Dashboard',
...
router.beforeEach(async (to, _from, next) => {
  ...
  if ((to.meta.requiresPayment || to.meta.requiresRiskControl || to.meta.requiresAffiliate || to.meta.requiresSubscription) && !appStore.publicSettingsLoaded) {
    try {
      await appStore.fetchPublicSettings()
    } catch (error) {
      console.warn('Failed to load public settings in route guard', error)
