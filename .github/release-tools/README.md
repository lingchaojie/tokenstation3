...
Pushes to the local `release` branch retain the `v0.1000.<run_number>` annotated auto-tag. The tag is bound to the triggering commit, and a rerun rejects any existing tag pointing elsewhere. Branch releases skip VERSION writeback to the default branch; tag releases retain it. Manual dry runs create no tag and do not publish.
...
Helper checks (Python 3.11 or newer; CI uses 3.12):
...
bash -n .github/release-tools/release-images.sh .github/release-tools/resolve-release-ref.sh
```
