...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	rewardCreditRepository := repository.NewRewardCreditRepository(client, db)
	billingCacheService := service.ProvideBillingCacheService(billingCache, userRepository, userSubscriptionRepository, apiKeyRepository, userRPMCache, userGroupRateRepository, configConfig, serviceUserPlatformQuotaRepository, rewardCreditRepository)
	userAPIKeyRouteRepository := repository.NewUserAPIKeyRouteRepository(client)
	apiKeyCache := repository.NewAPIKeyCache(redisClient)
	concurrencyCache := repository.ProvideConcurrencyCache(redisClient, configConfig)
	schedulerCache := repository.ProvideSchedulerCache(redisClient, configConfig)
	accountRepository := repository.NewAccountRepository(client, db, schedulerCache)
	concurrencyService := service.ProvideConcurrencyService(concurrencyCache, accountRepository, configConfig)
	apiKeyService := service.ProvideAPIKeyService(apiKeyRepository, userRepository, groupRepository, userSubscriptionRepository, userGroupRateRepository, userAPIKeyRouteRepository, settingService, apiKeyCache, configConfig, billingCacheService, concurrencyService)
	...
	rewardCreditService := service.NewRewardCreditService(rewardCreditRepository, apiKeyAuthCacheInvalidator, billingCache)
	userHandler := handler.ProvideUserHandler(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, rewardCreditService)
	...
	captureSidecarSupervisor := service.ProvideCaptureSidecarSupervisor(configConfig)
	conversationCapturePool := service.NewConversationCapturePool(configConfig, captureHealthRepository, settingService, captureSidecarSupervisor)
	...
	cursorOAuthClient := repository.NewCursorOAuthClient()
	cursorOAuthService := service.ProvideCursorOAuthService(proxyRepository, cursorOAuthClient, configConfig)
	cursorTokenProvider := service.ProvideCursorTokenProvider(accountRepository, geminiTokenCache, cursorOAuthService, oAuthRefreshAPI)
	openAIGatewayService := service.ProvideOpenAIGatewayService(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, conversationCapturePool, cursorTokenProvider, v...)
	...
	antigravityGatewayService := service.NewAntigravityGatewayService(accountRepository, gatewayCache, schedulerSnapshotService, antigravityTokenProvider, rateLimitService, httpUpstream, settingService, internal500CounterCache, conversationCapturePool)
	geminiMessagesCompatService := service.NewGeminiMessagesCompatService(accountRepository, groupRepository, gatewayCache, schedulerSnapshotService, geminiTokenProvider, rateLimitService, httpUpstream, antigravityGatewayService, configConfig, conversationCapturePool)
	...
	adminService := service.NewAdminService(userRepository, adminGroupRepository, adminAccountRepository, proxyRepository, apiKeyRepository, redeemCodeRepository, userGroupRateRepository, userAPIKeyRouteRepository, userRPMCache, billingCacheService, proxyExitInfoProber, proxyLatencyCache, apiKeyAuthCacheInvalidator, client, settingService, subscriptionService, userSubscriptionRepository, privacyClientFactory, openAIGatewayService, affiliateService, channelService)
	...
	accountTestService := service.ProvideAccountTestService(accountRepository, geminiTokenProvider, claudeTokenProvider, kiroTokenProvider, grokTokenProvider, antigravityGatewayService, httpUpstream, configConfig, tlsFingerprintProfileService, openAIGatewayService, settingService, cursorTokenProvider)
	crsSyncService := service.NewCRSSyncService(accountRepository, proxyRepository, oAuthService, openAIOAuthService, geminiOAuthService, configConfig)
	accountHandler := admin.ProvideAccountHandler(adminService, oAuthService, openAIOAuthService, geminiOAuthService, antigravityOAuthService, kiroOAuthService, grokOAuthService, rateLimitService, accountUsageService, accountTestService, concurrencyService, crsSyncService, sessionLimitCache, rpmCache, compositeTokenCacheInvalidator, grokQuotaService, v...)
	...
	captureAdminService := service.NewCaptureAdminService(configConfig, settingService, conversationCapturePool, captureHealthRepository, captureSidecarSupervisor)
	captureHandler := admin.NewCaptureHandler(captureAdminService)
	...
	checkInService := service.NewCheckInService(checkInRepository, settingService, apiKeyAuthCacheInvalidator, billingCache)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, kiroOAuthHandler, grokOAuthHandler, cursorOAuthHandler, cursorOAuthService, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, captureHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, affiliateHandler, checkInHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	usageRecordWorkerPool := service.NewUsageRecordWorkerPool(configConfig)
	userMsgQueueCache := repository.NewUserMsgQueueCache(redisClient)
	userMessageQueueService := service.ProvideUserMessageQueueService(userMsgQueueCache, rpmCache, configConfig)
	coordinator := handler.ProvideSecurityAuditCoordinator(contentModerationService)
	gatewayHandler := handler.ProvideGatewayHandler(gatewayService, openAIGatewayService, geminiMessagesCompatService, antigravityGatewayService, userService, concurrencyService, billingCacheService, usageService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, userMessageQueueService, configConfig, settingService, coordinator, conversationCapturePool)
	openAIGatewayHandler := handler.ProvideOpenAIGatewayHandler(openAIGatewayService, concurrencyService, billingCacheService, apiKeyService, usageRecordWorkerPool, errorPassthroughService, contentModerationService, opsService, grokQuotaService, configConfig, coordinator, conversationCapturePool)
	...
	webChatRepository := repository.NewWebChatRepository(client)
	localWebChatStorage := service.NewLocalWebChatStorageFromConfig(configConfig)
	accountService := service.NewAccountService(accountRepository, groupRepository)
	webChatService := service.NewWebChatService(webChatRepository, localWebChatStorage, userRepository, apiKeyService, subscriptionService, billingCacheService, gatewayService, openAIGatewayService, geminiMessagesCompatService, usageLogRepository, settingService, accountService, configConfig)
	webChatHandler := handler.ProvideWebChatHandler(webChatService)
	...
	subscriptionExpiryService := service.ProvideSubscriptionExpiryService(userSubscriptionRepository, settingRepository, notificationEmailService, leaderLockCache, db, paymentConfigService)
