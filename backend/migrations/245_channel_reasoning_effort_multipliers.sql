-- Migrate only explicitly configured legacy max pricing. Implicit model defaults remain in the service.
...
COMMENT ON COLUMN channel_model_pricing.reasoning_effort_multipliers IS
    'Custom billing multipliers by reasoning effort; omitted efforts use model defaults';
COMMENT ON COLUMN channel_account_stats_model_pricing.reasoning_effort_multipliers IS
    'Custom account statistics billing multipliers by reasoning effort; omitted efforts use model defaults';
