...
func TestMigration180AllowsCyberBlockedUsageRequestType(t *testing.T) {
	...
	for i, entry := range entries {
		switch entry.Name() {
		case "179_enable_grok_media_generation_groups.sql":
			previousIndex = i
		case "180_allow_cyber_blocked_usage_request_type.sql":
			currentIndex = i
		}
	}
	...
	content, err := FS.ReadFile("180_allow_cyber_blocked_usage_request_type.sql")
	require.NoError(t, err)
