...
-- 2. channel_monitors / channel_monitor_request_templates provider CHECK
--
-- Runs after 242_add_minimax_platform.sql. DROP ... IF EXISTS + 幂等守卫保证可重入；
-- 新约束是 242 的超集，必须同时保留 KIRO 与 MiniMax。
...
ALTER TABLE user_platform_quotas
    ...
    CHECK (platform IN ('anthropic', 'openai', 'kiro', 'gemini', 'antigravity', 'grok',
