...
func TestChannelImageInputPriceMigration(t *testing.T) {
	content, err := FS.ReadFile("230_channel_image_input_price.sql")
	require.NoError(t, err)
	...
	for _, table := range []string{
		...
		require.Contains(t, sql, "ALTER TABLE "+table+" ADD COLUMN IF NOT EXISTS image_input_price NUMERIC(20,12)")
	}
}
