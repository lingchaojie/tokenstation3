...
func ResponsesToAnthropicRequestWithOptions(req *ResponsesRequest, options ResponsesToAnthropicOptions) (*AnthropicRequest, error) {
	system, messages, err := convertResponsesInputToAnthropicWithCompaction(req.Instructions, req.Input, options.EnableCompaction, claude.IsOpus55(req.Model))
	if err != nil {
		return nil, err
	}
...
func convertResponsesInputToAnthropic(instructions string, inputRaw json.RawMessage, preserveThinking bool) (json.RawMessage, []AnthropicMessage, error) {
	return convertResponsesInputToAnthropicWithCompaction(instructions, inputRaw, false, preserveThinking)
}

func convertResponsesInputToAnthropicWithCompaction(instructions string, inputRaw json.RawMessage, enableCompaction, preserveThinking bool) (json.RawMessage, []AnthropicMessage, error) {
