...
func TestStream_RefusalLifecyclePreservesMixedContentOrder(t *testing.T) {
	events := collectStreamEvents(t, []string{
		`{"id":"mixed","choices":[{"index":0,"delta":{"role":"assistant","refusal":"blocked"}}]}`,
		`{"id":"mixed","choices":[{"index":0,"delta":{"content":"visible"}}]}`,
		`{"id":"mixed","choices":[{"index":0,"delta":{},"finish_reason":"content_filter"}],"usage":{"prompt_tokens":2,"completion_tokens":1}}`,
	})
	...
	for _, event := range events {
		...
		case "response.refusal.delta":
			require.Equal(t, "blocked", event.Delta)
			require.Equal(t, 0, event.ContentIndex)
		case "response.output_text.delta":
			require.Equal(t, "visible", event.Delta)
			require.Equal(t, 1, event.ContentIndex)
		case "response.output_item.done":
	...
	require.Contains(t, types, "response.refusal.delta")
	require.Contains(t, types, "response.refusal.done")
}
