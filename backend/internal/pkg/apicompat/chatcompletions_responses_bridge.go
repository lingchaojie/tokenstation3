	...
	for _, choice := range chunk.Choices {
		...
		if choice.Delta.Refusal != nil && *choice.Delta.Refusal != "" {
			...
			events = append(events, chatToResponsesEvent(state, "response.refusal.delta", &ResponsesStreamEvent{
				OutputIndex:  state.MessageIndex,
				ContentIndex: state.RefusalContentIndex,
	...
	if state.MessageItemID != "" {
		...
		events = append(events, chatToResponsesEvent(state, "response.output_item.done", &ResponsesStreamEvent{
			...
			Item: &ResponsesOutput{
				...
				Role:    "assistant",
				Content: state.chatMessageContent(),
				Status:  "completed",
			},
		}))
	}
...
func ensureChatToResponsesRefusalPart(state *ChatCompletionsToResponsesStreamState) []ResponsesStreamEvent {
	...
	if state.RefusalContentIndex < 0 {
...
func (state *ChatCompletionsToResponsesStreamState) chatMessageContent() []ResponsesContentPart {
	if state == nil {
		return nil
	}
	if state.nextContentIndex == 0 {
		return []ResponsesContentPart{{Type: "output_text", Text: state.Text.String()}}
	}
	content := make([]ResponsesContentPart, state.nextContentIndex)
	if state.TextContentIndex >= 0 {
		content[state.TextContentIndex] = ResponsesContentPart{Type: "output_text", Text: state.Text.String()}
	}
	if state.RefusalContentIndex >= 0 {
		content[state.RefusalContentIndex] = ResponsesContentPart{Type: "refusal", Refusal: state.Refusal.String()}
	}
	return content
}
