...
func TestGetAllGroupCapacityBatchRPMUsesSupportedAccountsOnly(t *testing.T) {
	accountRepo := &groupCapacityAccountRepoStub{
		rows: []GroupAccountCapacityRow{
			{
				GroupID:   10,
	...
	rpmCache := &groupCapacityRPMCacheStub{counts: map[int64]int{
		1: 3,
		2: 4,
		3: 5,
		4: 6,
	...
	require.Equal(t, []int64{1, 2}, rpmCache.requested)
	require.Equal(t, []GroupCapacitySummary{
		{GroupID: 10, RPMUsed: 7, RPMMax: 30},
	}, results)
}
