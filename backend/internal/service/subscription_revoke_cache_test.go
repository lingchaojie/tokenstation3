...
func (r *restoreUserSubRepoStub) ExistsGenericByUserID(context.Context, int64) (bool, error) {
	r.genericChecks++
	return r.existsGeneric, nil
}
...
func TestRestoreSubscription_GenericLiveSubscriptionConflict(t *testing.T) {
	deletedAt := time.Now().Add(-time.Hour)
	repo := &restoreUserSubRepoStub{
		existsGeneric: true,
		sub: &UserSubscription{
	...
	_, err := svc.RestoreSubscription(context.Background(), 1)
	require.ErrorIs(t, err, ErrSubscriptionRestoreConflict)
	require.Equal(t, 1, repo.genericChecks)
	require.Zero(t, repo.restoreCalls)
}
