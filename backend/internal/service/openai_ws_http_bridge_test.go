...
func TestOpenAIWSHTTPBridgeAcceptsFirstFrameAboveLegacy16MiB(t *testing.T) {
	gin.SetMode(gin.TestMode)
	const largeFrameRaceTimingBudget = 30 * time.Second
	...
	wsServer := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		readCtx, cancelRead := context.WithTimeout(r.Context(), largeFrameRaceTimingBudget)
		msgType, firstMessage, err := conn.Read(readCtx)
		cancelRead()
		if err != nil {
			errCh <- err
		...
		proxyCtx, cancelProxy := context.WithTimeout(r.Context(), largeFrameRaceTimingBudget)
		defer cancelProxy()
	...
	writeCtx, cancelWrite := context.WithTimeout(context.Background(), largeFrameRaceTimingBudget)
	...
	for {
		readCtx, cancelRead := context.WithTimeout(context.Background(), largeFrameRaceTimingBudget)
		msgType, event, readErr := clientConn.Read(readCtx)
		cancelRead()
		require.NoError(t, readErr)
		require.Equal(t, coderws.MessageText, msgType)
	...
	case proxyErr := <-errCh:
		require.NoError(t, proxyErr)
	case <-time.After(largeFrameRaceTimingBudget):
		t.Fatal("timed out waiting for websocket bridge proxy to finish")
	}
