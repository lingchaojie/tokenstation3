...
//  3. 空候选按 ErrModelPricingUnavailable 处理并拒绝成功零成本落账；
//  4. Responses 入站会分流到显式 anthropic 协议，且不误扩到 adaptive；
//  5. Responses×anthropic 流式转换器客户端断开后继续排水、usage 汇总完整。
...
func TestFilterCNProviderBillingModelCandidates(t *testing.T) {
	...
	require.Empty(t, allClaude, "全 claude 候选应被清空（上层必须按无法计费错误处理）")
...
func TestCalculateOpenAIRecordUsageCost_EmptyCandidatesIsPricingUnavailable(t *testing.T) {
	...
	require.True(t, isUsagePricingUnavailableError(err),
		"空候选必须按无价可循处理，阻止成功零成本落账: %v", err)
}

func TestShouldForwardOpenAIResponsesViaNativeAnthropic(t *testing.T) {
	for _, platform := range []string{PlatformKimi, PlatformZhipu, PlatformDeepseek} {
		account := &Account{
			Platform:    platform,
		...
		require.True(t, shouldForwardOpenAIResponsesViaNativeAnthropic(account), platform)
	}

	require.False(t, shouldForwardOpenAIResponsesViaNativeAnthropic(&Account{
		Platform:    PlatformKimi,
		Type:        AccountTypeAPIKey,
		Credentials: map[string]any{"api_protocol": APIProtocolAdaptive},
	}), "adaptive 协议仍由既有探测/协议路由决定，不在本次补线中扩大")
	require.False(t, shouldForwardOpenAIResponsesViaNativeAnthropic(nil))
}
