...
func (s *GatewayService) handleBedrockStreamingResponse(
	...
	defer func() {
		close(done)
		// A timeout may close the transport while Decode is blocked. Wait for the
		// decoder before publishing capture so bytes returned by that final Read
		// are included in the immutable response snapshot.
		closeCaptureResponseAndJoinScanner(resp, decodeDone)
	}()
