...
const (
	...
	PricingSourceOverrideFile = "override_file"
	PricingSourceFallback     = "fallback"
)

// PricingBucketProvenance records the winning source independently for every
// billable bucket. Empty means that bucket has no approved price.
type PricingBucketProvenance struct {
	Input                           string
	InputPriority                   string
	Output                          string
	OutputPriority                  string
	CacheWrite                      string
	CacheWritePriority              string
	CacheWrite5m                    string
	CacheWrite1h                    string
	CacheWrite5mPriority            string
	CacheWrite1hPriority            string
	CacheRead                       string
	CacheReadPriority               string
	ImageInput                      string
	ImageOutput                     string
	PerRequest                      string
	LongContext                     string // legacy aggregate source; retained for compatibility
	LongContextThreshold            string
	LongContextInput                string
	LongContextOutput               string
	LongContextCacheWrite5m         string
	LongContextCacheWrite1h         string
	LongContextCacheWrite5mPriority string
	LongContextCacheWrite1hPriority string
	LongContextCacheRead            string
	LongContextCacheReadPriority    string
}
...
type ResolvedPricing struct {
	...
	// defaultPerRequestPriceConfigured 区分“未配置”和“显式配置为 0（免费）”。
	...
	// basePricingMissing 保留解析来源信息，避免把未知模型构造出的空价格对象
	// 误当作真实的全局定价。
	basePricingMissing bool
...
func (r *ModelPricingResolver) Resolve(ctx context.Context, input PricingInput) *ResolvedPricing {
	...
	resolved := &ResolvedPricing{
		...
		basePricingMissing:     basePricing == nil,
		Provenance:             provenanceForBase(basePricing, source),
	}
	resolved.longContextPricingEnabled = longContextPricingEnabled
...
func (r *ModelPricingResolver) resolveConfiguredPricing(config *ChannelModelPricing, model, source string) *ResolvedPricing {
	...
	resolved.BasePricing, baseSource = r.resolveBasePricing(model)
	resolved.Provenance = provenanceForBase(resolved.BasePricing, baseSource)
	resolved.basePricingMissing = resolved.BasePricing == nil
...
func provenanceForBase(pricing *ModelPricing, source string) PricingBucketProvenance {
	if pricing == nil {
		return PricingBucketProvenance{}
	}
	p := pricing.provenance
	if source == PricingSourceFallback && p.Input == "" && p.Output == "" {
		// Approved built-ins predate pointer-presence metadata and are audited as
		// complete cards; bucket validation still decides whether a used price is valid.
		p = PricingBucketProvenance{
			Input: source, InputPriority: source, Output: source, OutputPriority: source,
			CacheWrite: source, CacheWritePriority: source, CacheRead: source, CacheReadPriority: source,
			CacheWrite5m: source, CacheWrite5mPriority: source,
			ImageInput: source, ImageOutput: source, LongContext: source,
			LongContextThreshold: source, LongContextInput: source, LongContextOutput: source,
		}
	}
	return p
}
...
func (r *ModelPricingResolver) applyTokenOverrides(chPricing *ChannelModelPricing, resolved *ResolvedPricing) {
	if resolved.BasePricing == nil {
		resolved.BasePricing = &ModelPricing{tokenPricePresenceKnown: true}
	} else {
	...
	overrideSource := resolved.Source
	if chPricing.InputPrice != nil {
		resolved.Provenance.Input = overrideSource
		resolved.BasePricing.provenance.Input = overrideSource
		if resolved.BasePricing.inputPriorityPriceConfigured {
			resolved.Provenance.InputPriority = overrideSource
			resolved.BasePricing.provenance.InputPriority = overrideSource
		}
	}
	if chPricing.OutputPrice != nil {
		resolved.Provenance.Output = overrideSource
		resolved.BasePricing.provenance.Output = overrideSource
		if resolved.BasePricing.outputPriorityPriceConfigured {
			resolved.Provenance.OutputPriority = overrideSource
			resolved.BasePricing.provenance.OutputPriority = overrideSource
		}
	}
	if chPricing.CacheWritePrice != nil {
		resolved.Provenance.CacheWrite = overrideSource
		resolved.Provenance.CacheWrite5m = overrideSource
		resolved.Provenance.CacheWrite1h = overrideSource
		resolved.BasePricing.provenance.CacheWrite = overrideSource
		resolved.BasePricing.provenance.CacheWrite5m = overrideSource
		resolved.BasePricing.provenance.CacheWrite1h = overrideSource
		if resolved.BasePricing.cacheWritePriorityConfigured {
			resolved.Provenance.CacheWritePriority = overrideSource
			resolved.Provenance.CacheWrite5mPriority = overrideSource
			resolved.BasePricing.provenance.CacheWritePriority = overrideSource
			resolved.BasePricing.provenance.CacheWrite5mPriority = overrideSource
		}
		if resolved.BasePricing.cacheWrite1hPriorityConfigured {
			resolved.Provenance.CacheWrite1hPriority = overrideSource
			resolved.BasePricing.provenance.CacheWrite1hPriority = overrideSource
		}
	}
	if chPricing.CacheReadPrice != nil {
		resolved.Provenance.CacheRead = overrideSource
		resolved.BasePricing.provenance.CacheRead = overrideSource
		if resolved.BasePricing.cacheReadPriorityConfigured {
			resolved.Provenance.CacheReadPriority = overrideSource
			resolved.BasePricing.provenance.CacheReadPriority = overrideSource
		}
	}
	...
	if chPricing.ImageOutputPrice != nil {
		...
		resolved.Provenance.ImageOutput = overrideSource
		resolved.BasePricing.provenance.ImageOutput = overrideSource
	}
	if chPricing.ImageInputPrice != nil {
		resolved.BasePricing.ImageInputPricePerToken = *chPricing.ImageInputPrice
		resolved.BasePricing.ImageInputPriceExplicit = true
		resolved.Provenance.ImageInput = overrideSource
		resolved.BasePricing.provenance.ImageInput = overrideSource
	}
...
func (r *ModelPricingResolver) applyRequestTierOverrides(chPricing *ChannelModelPricing, resolved *ResolvedPricing) {
	...
	for i := range resolved.RequestTiers {
		if resolved.RequestTiers[i].PerRequestPrice != nil {
			resolved.Provenance.PerRequest = resolved.Source
			break
		}
	}
...
// HasResolvablePricing reports whether a configured Group/Channel card contains
// a real price source. Pointer presence is intentional: an explicit zero is a
// valid free price, while nil means the administrator did not configure it.
func (resolved *ResolvedPricing) HasResolvablePricing() bool {
	if resolved == nil {
		return false
	}
	...
	if !resolved.basePricingMissing && resolved.BasePricing != nil {
		return true
	}
	return configuredTokenPriceExists(resolved.channelPricing)
}

func configuredTokenPriceExists(pricing *ChannelModelPricing) bool {
	if pricing == nil {
		return false
	}
...
func (r *ModelPricingResolver) GetIntervalPricing(resolved *ResolvedPricing, totalContextTokens int) *ModelPricing {
	pricing, _ := r.GetIntervalPricingWithProvenance(resolved, totalContextTokens)
	return pricing
}

// GetIntervalPricingWithProvenance resolves the selected context tier and
// returns the winning source labels for that exact tier.
func (r *ModelPricingResolver) GetIntervalPricingWithProvenance(resolved *ResolvedPricing, totalContextTokens int) (*ModelPricing, PricingBucketProvenance) {
	if resolved == nil {
		return nil, PricingBucketProvenance{}
	}
	if len(resolved.Intervals) == 0 {
		return resolved.BasePricing, resolved.Provenance
	}
	...
	markIntervalProvenance(&p, iv, resolved.Source)
	pricing.provenance = p
	return pricing, p
}

func markIntervalProvenance(p *PricingBucketProvenance, iv *PricingInterval, source string) {
	if p == nil || iv == nil {
		return
	}
	if iv.InputPrice != nil || iv.InputMultiplier != nil {
		p.Input, p.InputPriority = source, source
	}
	if iv.OutputPrice != nil || iv.OutputMultiplier != nil {
		p.Output, p.OutputPriority = source, source
	}
	if iv.CacheWritePrice != nil || iv.CacheWriteMultiplier != nil {
		p.CacheWrite, p.CacheWritePriority = source, source
		p.CacheWrite5m, p.CacheWrite1h = source, source
		p.CacheWrite5mPriority, p.CacheWrite1hPriority = source, source
	}
	if iv.CacheReadPrice != nil || iv.CacheReadMultiplier != nil {
		p.CacheRead, p.CacheReadPriority = source, source
	}
}
...
func intervalToModelPricing(iv *PricingInterval, base *ModelPricing, chPricing *ChannelModelPricing) *ModelPricing {
	...
	if iv.CacheWritePrice != nil {
		...
		pricing.CacheCreation5mPricePriority = pricing.CacheCreationPricePerTokenPriority
		pricing.CacheCreation1hPricePriority = channelTierOverridePrice(base1h, base1hPriority, *iv.CacheWritePrice)
		...
		pricing.cacheWrite1hPriorityConfigured = channelTierOverrideConfigured(base1h, base1hPriority,
			base1h > 0 || pricing.cacheWrite1hPriceConfigured,
			base1hPriority > 0 || pricing.cacheWrite1hPriorityConfigured)
		...
		pricing.CacheCreation5mPricePriority = applyMultiplier(pricing.CacheCreation5mPricePriority, iv.CacheWriteMultiplier)
		pricing.CacheCreation1hPricePriority = applyMultiplier(pricing.CacheCreation1hPricePriority, iv.CacheWriteMultiplier)
	}
...
func (r *ModelPricingResolver) GetRequestTierPrice(resolved *ResolvedPricing, tierLabel string) float64 {
	price, _ := r.GetRequestTierPriceFound(resolved, tierLabel)
	return price
}

// GetRequestTierPriceFound preserves the difference between an absent price
// and an explicitly configured zero price.
func (r *ModelPricingResolver) GetRequestTierPriceFound(resolved *ResolvedPricing, tierLabel string) (float64, bool) {
...
func (r *ModelPricingResolver) GetRequestTierPriceByContext(resolved *ResolvedPricing, totalContextTokens int) float64 {
	price, _ := r.GetRequestTierPriceByContextFound(resolved, totalContextTokens)
	return price
}

// GetRequestTierPriceByContextFound preserves explicit zero prices.
func (r *ModelPricingResolver) GetRequestTierPriceByContextFound(resolved *ResolvedPricing, totalContextTokens int) (float64, bool) {
