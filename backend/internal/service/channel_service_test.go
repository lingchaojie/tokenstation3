...
func TestValidatePricingBillingMode(t *testing.T) {
	tests := []struct {
		...
		{
			name: "image interval with token price only - invalid",
			pricing: []ChannelModelPricing{{
			...
			wantErr: true,
			errMsg:  "media billing interval requires per_request_price",
		},
		{
			name: "video interval with explicit zero per request price - valid",
			pricing: []ChannelModelPricing{{
