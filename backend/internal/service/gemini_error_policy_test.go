		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if svc.rateLimitService != nil {
				...
				case ErrorPolicySkipped:
					...
					gotFailover = svc.skippedErrorPolicyFailoverError(c, account, resp, respBody, "req-test") != nil
					goto verify
...
func TestSkippedErrorPolicyFailoverError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			resp := &http.Response{StatusCode: tt.statusCode, Header: http.Header{"X-Request-Id": []string{"req-1"}}}
			failoverErr := svc.skippedErrorPolicyFailoverError(c, tt.account, resp, body, "req-1")
