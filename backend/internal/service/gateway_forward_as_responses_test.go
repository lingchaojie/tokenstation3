...
func TestHandleResponsesBufferedStreamingResponse_ToolArgumentsAreValidJSON(t *testing.T) {
	...
	_, err := (&GatewayService{}).handleResponsesBufferedStreamingResponse(resp, c, "claude-fable-5", "claude-fable-5", nil, time.Now(), false, apicompat.ResponsesClientToolMapping{})
	require.NoError(t, err)
...
func TestOpus55ResponsesSignedThinkingBufferedAndStreamed(t *testing.T) {
	...
	for _, stream := range []bool{false, true} {
		...
		var err error
		if stream {
			_, err = svc.handleResponsesStreamingResponse(resp, c, "public-opus", "claude-opus-5-5", nil, time.Now(), false, apicompat.ResponsesClientToolMapping{})
		} else {
			_, err = svc.handleResponsesBufferedStreamingResponse(resp, c, "public-opus", "claude-opus-5-5", nil, time.Now(), false, apicompat.ResponsesClientToolMapping{})
		}
		require.NoError(t, err)
