...
func TestChannelImageInputPriceMustNotBeNegative(t *testing.T) {
	negative := -0.000001
	err := checkPricesNotNegative(ChannelModelPricing{ImageInputPrice: &negative})
	require.Error(t, err)
	require.Contains(t, err.Error(), "image_input_price")
}
