...
func TestGetPanelRateLimitSettingsCachedErrorDisablesPreviouslyEnabledValue(t *testing.T) {
	repo := &panelRateLimitSettingRepo{getValueErr: errors.New("db unavailable")}
	svc := newPanelRateLimitTestService(repo)
	svc.storePanelRateLimitCache(PanelRateLimitSettings{Enabled: true, UserRPM: 100}, -1)
