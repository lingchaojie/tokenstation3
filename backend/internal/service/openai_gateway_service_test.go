...
func TestOpenAIStreamingPreambleOnlyMissingTerminalIsForwarded(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			StreamDataIntervalTimeout: 0,
...
func TestOpenAIStreamingPassthroughMissingTerminalEventIsForwarded(t *testing.T) {
	gin.SetMode(gin.TestMode)
	cfg := &config.Config{
		Gateway: config.GatewayConfig{
			MaxLineSize: defaultMaxLineSize,
...
func TestOpenAIBuildUpstreamRequestOpenAIPassthroughCompactVersionClampsOldUserAgent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses/compact", bytes.NewReader([]byte(`{"model":"gpt-5"}`)))
	c.Request.Header.Set("User-Agent", "codex_cli_rs/0.141.0 (Ubuntu 22.4.0; x86_64) xterm-256color")
...
func TestOpenAIBuildUpstreamRequestCompactVersionClampsOldCustomUserAgent(t *testing.T) {
	gin.SetMode(gin.TestMode)
	rec := httptest.NewRecorder()
	c, _ := gin.CreateTestContext(rec)
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses/compact", bytes.NewReader([]byte(`{"model":"gpt-5"}`)))
	...
	svc := &OpenAIGatewayService{}
	customUA := "codex-tui/0.142.0 (Mac OS 15.5.0; arm64) ghostty/1.3.1 (codex-tui; 0.142.0)"
	account := &Account{
		Type:        AccountTypeOAuth,
		Platform:    PlatformOpenAI,
		Credentials: map[string]any{"chatgpt_account_id": "chatgpt-acc", "user_agent": customUA},
	}

	req, err := svc.buildUpstreamRequest(c.Request.Context(), c, account, []byte(`{"model":"gpt-5"}`), "token", false, "", true)
	require.NoError(t, err)
	require.Equal(t, openai.SetCodexUserAgentVersion(customUA, codexCLIVersion), req.Header.Get("User-Agent"))
	require.Equal(t, codexCLIVersion, req.Header.Get("Version"))
}
