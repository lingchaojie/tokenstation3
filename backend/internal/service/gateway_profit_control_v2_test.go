...
type profitKiroTTLCache struct {
	*sentinelMissGatewayCache
	ttl time.Duration
}

func (c *profitKiroTTLCache) SetSessionAccountID(ctx context.Context, groupID int64, sessionHash string, accountID int64, ttl time.Duration) error {
	c.ttl = ttl
	return c.sentinelMissGatewayCache.SetSessionAccountID(ctx, groupID, sessionHash, accountID, ttl)
}

func TestGatewayProfitControlAfterAdmissionPreservesMixedKiroTTL(t *testing.T) {
	group := &Group{ID: 4, Platform: PlatformAnthropic}
	account := &Account{ID: 173, Platform: PlatformKiro, Type: AccountTypeOAuth, Extra: map[string]any{
		"mixed_scheduling": true, "kiro_sticky_session_ttl_seconds": 1200,
	}}
	cache := &profitKiroTTLCache{sentinelMissGatewayCache: &sentinelMissGatewayCache{
		mockGatewayCacheForPlatform: &mockGatewayCacheForPlatform{sessionBindings: map[string]int64{}},
	}}
	svc := &GatewayService{cache: cache}
	gate := &openAIProfitControlGate{groupID: group.ID, platform: group.Platform, threshold: 0.5}
	gateCtx := context.WithValue(context.Background(), openAIProfitControlGateCtxKey{}, gate)

	require.NoError(t, svc.BindStickySessionForAccountGroupAfterProfitAdmission(gateCtx, &group.ID, "mixed-kiro", account, group))
	require.Equal(t, 1200*time.Second, cache.ttl)
	require.Equal(t, account.ID, cache.sessionBindings["mixed-kiro"])
}
