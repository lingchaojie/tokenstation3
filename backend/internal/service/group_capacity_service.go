...
func (s *GroupCapacityService) getGroupCapacitiesBatch(ctx context.Context, groupIDs []int64, lister groupCapacityAccountLister) ([]GroupCapacitySummary, error) {
	...
	rpmRefs := make([]groupCapacityAccountRef, 0, len(rows))
	...
	rpmAccountIDs := accountIDsForGroupsWithLimit(rpmRefs, groupIndex, results, func(summary GroupCapacitySummary) bool {
