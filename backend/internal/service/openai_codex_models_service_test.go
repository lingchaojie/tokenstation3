...
func TestBuildCodexModelsManifestForGroupAdvertisesOfficialGrokResponsesImageInput(t *testing.T) {
	...
	body, err := svc.BuildCodexModelsManifestForGroup(
		context.Background(),
		&Group{ID: groupID, Platform: PlatformGrok},
		"",
		[]string{"grok-4.5"},
	)
	require.NoError(t, err)
...
func TestBuildCodexModelsManifestForGroupUsesProviderImageCapabilities(t *testing.T) {
	...
	for i, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			body, err := svc.BuildCodexModelsManifestForGroup(
				context.Background(),
				&Group{ID: groupID, Platform: tt.accounts[0].Platform},
				"",
				[]string{tt.model},
			)
			require.NoError(t, err)
...
// Scenario: an exact alias inherits metadata from its unique mapped target model.
func TestBuildCodexModelsManifestForGroupUsesMappedTargetMetadataForAlias(t *testing.T) {
	t.Parallel()
	...
	body, err := svc.BuildCodexModelsManifestForGroup(
		context.Background(),
		&Group{ID: groupID, Platform: PlatformAnthropic},
		"",
...
func TestBuildCodexModelsManifestForGroupFallsBackWhenCapabilityLookupFails(t *testing.T) {
	...
	body, err := svc.BuildCodexModelsManifestForGroup(
		context.Background(),
		&Group{ID: 734, Platform: PlatformOpenAI},
		"",
		[]string{"gpt-5.6-sol"},
	)
	require.NoError(t, err)
