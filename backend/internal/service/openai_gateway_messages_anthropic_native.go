...
func (s *OpenAIGatewayService) forwardAnthropicViaNativeAnthropicEndpoint(
	...
	// 记录实际转发的推理强度（包括 GLM-5.3 归一化）：优先 output_config.effort；
	// 缺失且 thinking 已启用时，按国产 passback-required 模型兜底为 high
	// （对齐 Anthropic 网关 gateway_handler 的记录语义，避免该路径长期落 NULL）。
	forwardedReasoningEffort := NormalizeClaudeOutputEffort(gjson.GetBytes(body, "output_config.effort").String())
	reasoningEffort := ApplyThinkingEnabledFallback(
		forwardedReasoningEffort,
		body,
		billingModel,
	)
		...
		case ev, ok := <-events:
			if !ok {
				if pendingStreamError != nil {
					return s.nativeAnthropicStreamResult(c, resp, usage, firstTokenMs, clientDisconnected, sawTerminalEvent, originalModel, billingModel, upstreamModel, reasoningEffort, startTime), pendingStreamError
				}
				if !clientDisconnected {
					flusher.Flush()
				}
				if !sawTerminalEvent {
					return s.nativeAnthropicStreamResult(c, resp, usage, firstTokenMs, clientDisconnected, sawTerminalEvent, originalModel, billingModel, upstreamModel, reasoningEffort, startTime),
						fmt.Errorf("stream usage incomplete: missing terminal event")
				}
				return s.nativeAnthropicStreamResult(c, resp, usage, firstTokenMs, clientDisconnected, sawTerminalEvent, originalModel, billingModel, upstreamModel, reasoningEffort, startTime), nil
			}
			if ev.err != nil {
				if sawTerminalEvent {
					return s.nativeAnthropicStreamResult(c, resp, usage, firstTokenMs, clientDisconnected, sawTerminalEvent, originalModel, billingModel, upstreamModel, reasoningEffort, startTime), nil
				...
				if errors.Is(ev.err, bufio.ErrTooLong) {
					...
					return s.nativeAnthropicStreamResult(c, resp, usage, firstTokenMs, clientDisconnected, sawTerminalEvent, originalModel, billingModel, upstreamModel, reasoningEffort, startTime), ev.err
				}
				return s.nativeAnthropicStreamResult(c, resp, usage, firstTokenMs, clientDisconnected, sawTerminalEvent, originalModel, billingModel, upstreamModel, reasoningEffort, startTime),
