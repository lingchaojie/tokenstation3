...
func (s *CNProviderBalanceService) queryBalanceForAccount(ctx context.Context, account *Account) (*CNProviderBalanceResult, error) {
	...
	bodyBytes, err := readUpstreamResponseBodyLimited(resp.Body, cnBalanceMaxBodyBytes)
	if err != nil {
		return nil, infraerrors.New(
			http.StatusBadGateway,
			"CN_BALANCE_RESPONSE_READ_FAILED",
			"failed to read balance response",
		).WithCause(err)
	}
	...
	if !gjson.ValidBytes(bodyBytes) {
		result.Error = "Balance API returned malformed JSON"
		return result, nil
	}
	...
	case PlatformKimi:
		...
		codeResult := gjson.GetBytes(bodyBytes, "code")
		code, codeOK := cnParseF64(codeResult.Value())
		statusResult := gjson.GetBytes(bodyBytes, "status")
		if !codeResult.Exists() || !codeOK || code != 0 || statusResult.Type != gjson.True {
			result.Error = "Kimi balance API returned an unsuccessful response"
			return result, nil
		}
		balanceResult := gjson.GetBytes(bodyBytes, "data.available_balance")
		balance, balanceOK := cnParseF64(balanceResult.Value())
		if !balanceResult.Exists() || !balanceOK || math.IsNaN(balance) || math.IsInf(balance, 0) {
			result.Error = "Kimi balance API response is missing a valid available_balance"
			return result, nil
		}
		entries = append(entries, CNProviderBalanceEntry{Currency: "CNY", Balance: balance})
	case PlatformDeepseek:
		availableResult := gjson.GetBytes(bodyBytes, "is_available")
		if availableResult.Type != gjson.True && availableResult.Type != gjson.False {
			result.Error = "DeepSeek balance API response is missing a valid is_available"
			return result, nil
		}
		available = availableResult.Bool()
		// balance_infos 逐条解析：双币种账号同时返回 CNY + USD（数组顺序即
		// 主次序，首条为主币种）。
		balanceInfos := gjson.GetBytes(bodyBytes, "balance_infos")
		if !balanceInfos.IsArray() || len(balanceInfos.Array()) == 0 {
			result.Error = "DeepSeek balance API response is missing balance_infos"
			return result, nil
		}
		for _, info := range balanceInfos.Array() {
			currency := strings.ToUpper(strings.TrimSpace(info.Get("currency").String()))
			balanceResult := info.Get("total_balance")
			balance, balanceOK := cnParseF64(balanceResult.Value())
			if currency == "" || !balanceResult.Exists() || !balanceOK || math.IsNaN(balance) || math.IsInf(balance, 0) {
				result.Error = "DeepSeek balance API response contains an invalid balance entry"
				return result, nil
			}
