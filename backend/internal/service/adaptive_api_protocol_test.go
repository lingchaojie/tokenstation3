...
func TestKimiOrdinaryChatKeepsChatCompletionsProtocol(t *testing.T) {
	gin.SetMode(gin.TestMode)
	for _, protocol := range []string{APIProtocolAdaptive, APIProtocolChatCompletions} {
		t.Run(protocol, func(t *testing.T) {
			body := []byte(`{"model":"k3-256k","messages":[{"role":"user","content":"hello"}],"stream":false}`)
			upstream := &httpUpstreamRecorder{err: errors.New("stop after capture")}
			svc := &OpenAIGatewayService{cfg: rawChatCompletionsTestConfig(), httpUpstream: upstream}
