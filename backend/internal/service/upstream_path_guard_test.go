...
func TestPathGuardSanitizedUpstreamPathSuffixRejectsNonConformingSegments(t *testing.T) {
...
func TestPathGuardSanitizedUpstreamPathSuffixEnforcesBounds(t *testing.T) {
	longSegment := "/"
...
// 也不会被误判成 compact 请求。
func TestPathGuardOpenAIResponsesRequestPathSuffixRejectsNonConformingSubpaths(t *testing.T) {
...
func TestPathGuardAppendOpenAIResponsesRequestPathSuffixRefusesUnsafeSuffix(t *testing.T) {
...
// KIRO relay accounts deliberately retain administrator-configured base URL path
// prefixes. The request suffix guard must validate only the client-controlled
// suffix and must not rewrite or reject that trusted relay prefix.
func TestPathGuardKiroRelayCustomEndpointPreservesPrefix(t *testing.T) {
	const relayResponsesURL = "https://relay.example.test/tenant-a/anthropic/v1/responses"

	c := newResponsesSuffixTestContext(t, "/v1/responses/compact")
	require.True(t, IsForwardableOpenAIResponsesRequestPath(c))
	require.Equal(t, relayResponsesURL+"/compact",
		appendOpenAIResponsesRequestPathSuffix(relayResponsesURL, openAIResponsesRequestPathSuffix(c)))
}
