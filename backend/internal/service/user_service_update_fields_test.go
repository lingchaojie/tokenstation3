...
func TestUpdateProfile_OnlyDeclaresRequestedColumns(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "subscription balance fallback only",
			req:  UpdateProfileRequest{SubscriptionBalanceFallbackEnabled: &fallback},
			want: UserUpdateFields{SubscriptionBalanceFallbackEnabled: true},
		},
		{
