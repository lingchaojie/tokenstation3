...
func ProvideUpdateService(cache UpdateCache, githubClient GitHubReleaseClient, buildInfo BuildInfo, cfg *config.Config) *UpdateService {
	...
	return NewUpdateServiceWithGitHubRepo(cache, githubClient, buildInfo.Version, buildInfo.BuildType, githubRepo)
}
...
func ProvideSettingService(settingRepo SettingRepository, groupRepo GroupRepository, proxyRepo ProxyRepository, cfg *config.Config) (*SettingService, error) {
	...
	if err := LoadGrokRuntimeModelMappingSettings(context.Background(), settingRepo); err != nil {
		return nil, fmt.Errorf("load Grok runtime model mapping settings: %w", err)
	}
