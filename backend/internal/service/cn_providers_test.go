...
func TestNormalizeDeepSeekResponsesRequestBody(t *testing.T) {
	...
	for _, protocol := range []string{"", APIProtocolChatCompletions} {
		kimiCC := &Account{
			Platform: PlatformKimi, Type: AccountTypeAPIKey,
			Credentials: map[string]any{"api_protocol": protocol},
		}
		require.Equal(t, string(body), string(normalizeDeepSeekResponsesRequestBody(kimiCC, body)))
	}
