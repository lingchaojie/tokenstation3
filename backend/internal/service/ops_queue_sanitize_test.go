...
func TestSanitizeOpsUpstreamErrorsForQueueBoundsAndRedacts(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		...
		entry.UpstreamErrors = append(entry.UpstreamErrors, &OpsUpstreamErrorEvent{
			...
			MappedModel:          strings.Repeat("m", 200),
			KiroModelID:          " kiro-sonnet ",
			HasTools:             true,
	...
	for i, event := range events {
		if event.RequestedModel != "claude-sonnet-4-6" || event.MappedModel != strings.Repeat("m", 128) || event.KiroModelID != "kiro-sonnet" || !event.HasTools {
			t.Fatalf("event %d lost or failed to sanitize local model diagnostics: %+v", i, event)
		}
