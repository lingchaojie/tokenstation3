...
func TestOpenAIAgentIdentityTaskInvalidRetriesExactlyOnce(t *testing.T) {
	...
	upstream.responses = []*http.Response{
		{StatusCode: http.StatusUnauthorized, Header: http.Header{"Content-Type": []string{"application/json"}}, Body: io.NopCloser(strings.NewReader(`{"error":{"code":"invalid_task_id"}}`))},
		{StatusCode: http.StatusOK, Header: http.Header{"Content-Type": []string{"text/event-stream"}}, Body: io.NopCloser(strings.NewReader("data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_task_recovered\",\"status\":\"completed\",\"output\":[],\"usage\":{\"input_tokens\":1,\"output_tokens\":1,\"total_tokens\":2}}}\n\ndata: [DONE]\n\n"))},
	}
...
func TestOpenAIAgentIdentityFailoverRedactsOpsMessage(t *testing.T) {
	gin.SetMode(gin.TestMode)
	key, privateKey := newTestAgentIdentityKey(t)
	account := &Account{
		ID:          24,
		Name:        "agent-identity-redaction",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeOAuth,
		Status:      StatusActive,
		Schedulable: true,
		...
		Credentials: map[string]any{
			...
			"agent_private_key":  privateKey,
			"task_id":            key.taskID,
			"chatgpt_account_id": "account-agent-redaction",
		},
	}
	leakedMessage := key.runtimeID + " " + key.taskID + " " + privateKey + " AgentAssertion abc123"
	errorBody, err := json.Marshal(map[string]any{
		"error": map[string]any{"code": "server_is_overloaded", "message": leakedMessage},
	})
	require.NoError(t, err)
	upstream := &httpUpstreamRecorder{resp: &http.Response{
	...
	require.NotContains(t, string(failoverErr.ResponseBody), key.runtimeID)
	require.NotContains(t, string(failoverErr.ResponseBody), key.taskID)
	require.NotContains(t, string(failoverErr.ResponseBody), privateKey)
	...
	for _, event := range events {
		...
		require.NotContains(t, event.Message, key.taskID)
		require.NotContains(t, event.Message, privateKey)
		require.NotContains(t, event.Message, "AgentAssertion abc123")
	}
}
