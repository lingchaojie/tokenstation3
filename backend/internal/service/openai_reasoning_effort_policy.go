	...
	for i, mapping := range raw {
		...
		if from != "none" {
			...
			return nil, fmt.Errorf(
				...
				i+1, PlatformOpenAI, PlatformAnthropic,
			)
		}
		if to != ReasoningEffortMappingDeny {
			...
			return nil, fmt.Errorf(
				"reasoning effort mapping %d target: reasoning effort policy is only supported for platforms %q and %q",
				i+1,
				PlatformAnthropic,
				PlatformOpenAI,
			)
...
func sanitizeGroupReasoningEffortPolicy(group *Group) {
	...
	if overLimit == "" || (group.Platform != PlatformAnthropic && group.Platform != PlatformOpenAI) {
