...
type AuditLogRepository interface {
	...
	// ClearAllWithTrace 在同一事务内锁表、统计、清空并写入留痕。
	// trace 为 nil 时只执行统计与清空。
...
func RedactAuditBody(raw []byte, contentType string) string {
	...
	if len(out) > auditRequestBodyMaxBytes {
		...
		limit := auditRequestBodyMaxBytes - len(marker)
		for limit > 0 && !utf8.ValidString(out[:limit]) {
			limit--
		}
		out = out[:limit] + marker
	}
	return out
}
