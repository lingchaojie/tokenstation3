...
func TestCalculateCostWithServiceTier_Gpt54MiniPriorityFallsBackToTierMultiplier(t *testing.T) {
	svc := newTestBillingService()
	// Mini has no approved cache-write bucket; service-tier coverage therefore
	// uses only buckets present on its audited card.
	tokens := UsageTokens{InputTokens: 120, OutputTokens: 30, CacheReadTokens: 8}
...
func TestCalculateCostWithServiceTier_Gpt54NanoFlexAppliesHalfMultiplier(t *testing.T) {
	svc := newTestBillingService()
	// Nano has no approved cache-write bucket; service-tier coverage therefore
	// uses only buckets present on its audited card.
	tokens := UsageTokens{InputTokens: 100, OutputTokens: 50, CacheReadTokens: 20}
	...
	svc := NewBillingService(&config.Config{}, &PricingService{
		pricingData: map[string]*LiteLLMModelPricing{
			"dynamic-tier-model": {
				...
				CacheCreationInputTokenCostAbove1hrConfigured: true,
...
func TestGetModelPricingWithChannel_NilImageOutputPricePreservesBaseBucket(t *testing.T) {
	svc := newTestBillingService()
...
func TestValidateModelPricingForUsage_ExplicitZeroVersusMissingByBucket(t *testing.T) {
	tests := []struct {
		name      string
		tokens    UsageTokens
		configure func(*ModelPricing)
	}{
		{name: "input", tokens: UsageTokens{InputTokens: 1}, configure: func(p *ModelPricing) { p.inputPriceConfigured = true }},
		{name: "output", tokens: UsageTokens{OutputTokens: 1}, configure: func(p *ModelPricing) { p.outputPriceConfigured = true }},
		{name: "cache_write", tokens: UsageTokens{CacheCreationTokens: 1}, configure: func(p *ModelPricing) { p.cacheWritePriceConfigured = true }},
		{name: "cache_read", tokens: UsageTokens{CacheReadTokens: 1}, configure: func(p *ModelPricing) { p.cacheReadPriceConfigured = true }},
		{name: "image_input", tokens: UsageTokens{InputTokens: 1, ImageInputTokens: 1}, configure: func(p *ModelPricing) { p.ImageInputPriceExplicit = true }},
		{name: "image_output", tokens: UsageTokens{OutputTokens: 1, ImageOutputTokens: 1}, configure: func(p *ModelPricing) { p.ImageOutputPriceExplicit = true }},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			missing := &ModelPricing{tokenPricePresenceKnown: true}
			require.ErrorIs(t, validateModelPricingForUsage(missing, tc.tokens, "", true), ErrModelPricingUnavailable)

			explicitZero := &ModelPricing{tokenPricePresenceKnown: true}
			tc.configure(explicitZero)
			require.NoError(t, validateModelPricingForUsage(explicitZero, tc.tokens, "", true))
		})
	}
}
