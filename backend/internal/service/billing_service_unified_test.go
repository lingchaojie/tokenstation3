...
func TestCalculateCostUnified_EmptyTokenCardWithoutBasePricingFailsClosed(t *testing.T) {
	bs := newTestBillingService()
	resolver := NewModelPricingResolver(nil, bs)
	group := &Group{ModelPricing: []ChannelModelPricing{{
...
func TestCalculateCostUnified_HistoricalChannelCacheEmptyTokenCardFailsClosed(t *testing.T) {
	groupID := int64(12)
	channelService := newTestChannelServiceWithCache(t, &channelCache{
		pricingByGroupModel: map[channelModelKey]*ChannelModelPricing{
			{groupID: groupID, model: "cached-customer-alias"}: {
				BillingMode: BillingModeToken,
			},
		},
		channelByGroupID: map[int64]*Channel{
...
func TestCalculateCostUnified_ExplicitZeroTokenPriceIsConfiguredFree(t *testing.T) {
	bs := newTestBillingService()
	resolver := NewModelPricingResolver(nil, bs)
	group := &Group{ModelPricing: []ChannelModelPricing{{
		Models:      []string{"customer-alias"},
...
func TestCalculateCostUnified_MissingRelevantTokenBucketFailsClosed(t *testing.T) {
	bs := newTestBillingService()
	resolver := NewModelPricingResolver(nil, bs)
	group := &Group{ModelPricing: []ChannelModelPricing{{
		Models:      []string{"customer-alias"},
...
func TestCalculateCostUnified_ZeroImageInputOverrideWithoutTextInputPriceFailsClosed(t *testing.T) {
	bs := newTestBillingService()
	resolver := NewModelPricingResolver(nil, bs)
	group := &Group{ModelPricing: []ChannelModelPricing{{
		Models:          []string{"customer-image-alias"},
		BillingMode:     BillingModeToken,
...
func TestCalculateCostUnified_MediaTierWithoutPerRequestPriceFailsClosed(t *testing.T) {
	bs := newTestBillingService()
	resolver := NewModelPricingResolver(nil, bs)
	group := &Group{ModelPricing: []ChannelModelPricing{{
...
func TestCalculateCostUnified_ExplicitZeroMediaTierDoesNotFallBackToDefault(t *testing.T) {
	bs := newTestBillingService()
	resolver := NewModelPricingResolver(nil, bs)
	group := &Group{ModelPricing: []ChannelModelPricing{{
		Models:          []string{"free-image-tier"},
		BillingMode:     BillingModeImage,
