...
func TestModelReasoningEffortDefaultsAndOverrides(t *testing.T) {
	for _, model := range []string{"claude-fable-5-1", "vendor/claude-fable-5.1-20260901", "fable51-thinking"} {
		for _, multipliers := range []map[string]float64{nil, {}, {"high": 2}} {
			require.Equal(t, 3.0, modelReasoningEffortBillingMultiplier(model, " MAX ", multipliers))
			require.Equal(t, 1.0, modelReasoningEffortBillingMultiplier(model, "xhigh", multipliers))
		}
		for _, override := range []float64{0.5, 1, 1.5, 4} {
			require.Equal(t, override, modelReasoningEffortBillingMultiplier(model, "max", map[string]float64{"max": override}))
		}
	}
	for _, model := range []string{"claude-fable-5", "claude-fable-5-11", "gpt-6-sol", "custom-model"} {
		require.Equal(t, 1.0, modelReasoningEffortBillingMultiplier(model, "max", nil))
		require.Equal(t, 2.0, modelReasoningEffortBillingMultiplier(model, "high", map[string]float64{"high": 2}))
	}
}

func TestReasoningEffortPreservesDefaultOverrideAndAccountStatsParity(t *testing.T) {
	for _, multipliers := range []map[string]float64{nil, {}, {"high": 2}, {"max": 1}, {"max": 1.5}} {
		pricing := ChannelModelPricing{
			...
			CacheWritePrice: testPtrFloat64(3e-6), CacheWrite1hPrice: testPtrFloat64(4e-6), CacheReadPrice: testPtrFloat64(0),
			ImageInputPrice: testPtrFloat64(0), ImageOutputPrice: testPtrFloat64(5e-6),
			ReasoningEffortMultipliers: multipliers,
		}
		bs, resolver := newTokenCostTestEnv(t, PlatformAnthropic, []ChannelModelPricing{pricing}, nil)
		...
		tokens := UsageTokens{InputTokens: 100, ImageInputTokens: 20, OutputTokens: 30, ImageOutputTokens: 10, CacheCreationTokens: 30, CacheCreation5mTokens: 10, CacheCreation1hTokens: 20, CacheReadTokens: 50}
		for _, effort := range []string{"max", "high", "xhigh"} {
			cost, err := bs.CalculateTokenCostForRequest(TokenCostRequest{Ctx: context.Background(), Model: "claude-fable-5-1", Group: group, Tokens: tokens, RateMultiplier: 1, ReasoningEffort: effort, Resolver: resolver})
			require.NoError(t, err)
			stats := tryCustomRulesWithUsage(channel, 1, 100, PlatformAnthropic, "claude-fable-5-1", tokens, accountStatsCostUsage{requestCount: 1}, effort)
			require.NotNil(t, stats)
			require.InDelta(t, cost.TotalCost, *stats, 1e-12)
			require.InDelta(t, 0.00028*modelReasoningEffortBillingMultiplier("claude-fable-5-1", effort, multipliers), cost.TotalCost, 1e-12)
...
func TestReasoningEffortNoResolverFailsClosedForMissingBucket(t *testing.T) {
	catalog := &PricingService{}
	var err error
	catalog.pricingData, err = catalog.parsePricingData([]byte(`{"custom-model":{"input_cost_per_token":0}}`))
	require.NoError(t, err)
	bs := NewBillingService(&config.Config{}, catalog)
	_, err = bs.CalculateCostUnified(CostInput{Model: "custom-model", Tokens: UsageTokens{OutputTokens: 1}, ReasoningEffort: "max", RateMultiplier: 1})
	require.ErrorIs(t, err, ErrModelPricingUnavailable)
	cost, err := bs.CalculateCostUnified(CostInput{Model: "custom-model", Tokens: UsageTokens{InputTokens: 1}, ReasoningEffort: "max", RateMultiplier: 1})
	require.NoError(t, err)
	require.Zero(t, cost.TotalCost)
}

func TestReasoningEffortVideoDurationAndSizeTierStatsParity(t *testing.T) {
	for _, price := range []float64{0, 0.12} {
		pricing := ChannelModelPricing{
			Platform: PlatformOpenAI, Models: []string{"custom-video"}, BillingMode: BillingModeVideo,
			PerRequestPrice: testPtrFloat64(9), ReasoningEffortMultipliers: map[string]float64{"high": 1.5},
			Intervals: []PricingInterval{{TierLabel: "720p", PerRequestPrice: testPtrFloat64(price)}},
		}
		...
		cost, err := bs.CalculateCostUnified(CostInput{
			Ctx: context.Background(), Model: "custom-video", Group: group, GroupID: &group.ID, Resolver: resolver,
			RequestCount: 2, UsageUnits: 2.5, SizeTier: "720p", RateMultiplier: 0.8, ReasoningEffort: "high",
		})
		require.NoError(t, err)
		channel := &Channel{AccountStatsPricingRules: []AccountStatsPricingRule{{AccountIDs: []int64{1}, Pricing: []ChannelModelPricing{pricing}}}}
		stats := tryCustomRulesWithUsage(channel, 1, 100, PlatformOpenAI, "custom-video", UsageTokens{}, accountStatsCostUsage{requestCount: 2, usageUnits: 2.5, sizeTier: "720p"}, "high")
		require.NotNil(t, stats)
		require.InDelta(t, 2.5*price*1.5, cost.TotalCost, 1e-12)
		require.InDelta(t, cost.TotalCost, *stats, 1e-12)
