...
//   - 选号结果通过 AccountSelectionResult 携带真实生效的门（fallback
...
// 不同的分组（fallback 目标分组），turn 级重装以连接上已装门的调度分组为准；
...
// 装进 ctx。抑制标记、未启用/非 OpenAI 网关平台分组/无法取到分组配置时原样返回 ctx
...
func (s *OpenAIGatewayService) withOpenAIProfitControlGate(ctx context.Context, groupID *int64) context.Context {
	...
	if gate == nil {
		...
		//（fallback/模型路由等跨分组调度）。typed-nil 覆盖值由否决点按无门放行。
...
func (s *OpenAIGatewayService) resolveOpenAIProfitControlGate(ctx context.Context, groupID *int64) *openAIProfitControlGate {
	...
	// 直接复用 auth cache 分组，热路径零额外查询；fallback 路由到目标
...
// 选号函数内部的局部 ctx 上安装（fallback 路由还可能解析出与入口
