...
func finalizePostUsageBilling(ctx context.Context, p *postUsageBillingParams, deps *billingDeps, result *UsageBillingApplyResult) {
	...
	if !actualSubscriptionBill && p.Platform != "" && p.Cost.ActualCost > 0 && p.User != nil && deps.userPlatformQuotaRepo != nil && deps.billingCacheService != nil {
...
func (s *GatewayService) recordUsageCore(ctx context.Context, input *recordUsageCoreInput, opts *recordUsageOpts, validateOnly bool) error {
	...
	// 计算费用
	opts.IsKiroDirect = isKiroDirectModeAccount(account)
...
func shouldUseKiroConservativeBillingFallback(result *ForwardResult, billingModel string, opts *recordUsageOpts) bool {
	...
	return opts != nil && opts.IsKiroDirect
}
...
func (s *GatewayService) calculateTokenCost(
	...
	if (resolved == nil || (resolved.Source != PricingSourceGroup && resolved.Source != PricingSourceChannel)) &&
		opts != nil && opts.LongContextThreshold > 0 && (apiKey == nil || apiKey.Group == nil || apiKey.Group.LongContextPricingEnabled) {
		// Preserve the approved provider-specific Gemini fallback only when no
		// explicit site/group/channel price owns the request.
		...
		applyCostBreakdownMultiplier(cost, modelReasoningEffortBillingMultiplier(billingModel, optionalStringValue(result.ReasoningEffort), nil))
	} else {
		var group *Group
		if apiKey != nil {
			group = apiKey.Group
