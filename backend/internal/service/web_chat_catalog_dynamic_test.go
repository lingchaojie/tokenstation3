...
func TestResolveWebChatCatalog_EnrichesClaudeFable51FromPublicCatalog(t *testing.T) {
	gr := stubGroupResolver{ids: map[string]int64{APIKeyTypeAnthropic: 3}}
	al := stubAccountLister{byGroup: map[int64][]Account{
		3: {acctWithMapping(PlatformAnthropic, "claude-fable-5-1")},
	}}
	...
	if got[0].DisplayName != "Claude Fable 5.1" {
		t.Fatalf("display name=%q want Claude Fable 5.1", got[0].DisplayName)
	}
	if got[0].ReleasedAt != "2026-09-01" {
		t.Fatalf("released_at=%q want 2026-09-01", got[0].ReleasedAt)
	}
}
