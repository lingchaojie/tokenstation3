...
func (s *forwardedIPMigrationRepoStub) SetIfAbsent(_ context.Context, key, value string) error {
...
func TestSettingService_UpdateSettingsAllowsZeroOpenAIOAuthSchedulingRateMultiplier(t *testing.T) {
	repo := &settingUpdateRepoStub{}
	svc := NewSettingService(repo, &config.Config{})
...
func TestSettingService_PasskeySwitchPersistsAndDefaultsOff(t *testing.T) {
...
func TestSettingService_SessionBindingAndStepUpDefaultOff(t *testing.T) {
	service := NewSettingService(&forwardedIPMigrationRepoStub{values: map[string]string{}}, &config.Config{})

	require.False(t, service.IsSessionBindingEnabled(context.Background()))
	require.False(t, service.IsStepUpEnabled(context.Background()))
}
