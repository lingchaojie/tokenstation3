...
func expectedSchedulerBucketQueryCount(buckets []SchedulerBucket) int {
	cacheable := make(map[schedulerAccountQueryKey]struct{})
	count := 0
	for _, bucket := range buckets {
		if key, ok := schedulerAccountQueryKeyForBucket(bucket); ok {
			cacheable[key] = struct{}{}
			continue
		}
		count++
	}
	return count + len(cacheable)
}
...
func TestSchedulerGroupLifecycleActiveReopensAndRebuildsAllCurrentBuckets(t *testing.T) {
	...
	require.Equal(t, expectedSchedulerBucketQueryCount(current), accounts.callCount())
