...
func TestSchedulerBulkAccountEventScopesKiroAndCleansMixedAnthropicBucket(t *testing.T) {
	cache := newBulkEventSnapshotCache()
	// fresh 值可能已经关闭 mixed_scheduling；除 KIRO 自身桶外仍须重建
	// Anthropic mixed 桶以清理旧成员，但不得扩散到其他无关平台。
	repo := newBulkEventAccountRepo(&Account{ID: 12, Platform: PlatformKiro, GroupIDs: []int64{82}})
	svc := newBulkEventTestService(cache, repo)

	err := svc.handleBulkAccountEvent(context.Background(), bulkEventPayload([]int64{12}, []int64{81}), make(map[batchSeenKey]struct{}))

	require.NoError(t, err)
	require.ElementsMatch(t,
		schedulerBucketsForTest([]int64{81, 82}, PlatformAnthropic, PlatformKiro),
		cache.capturedBuckets(),
	)
}
