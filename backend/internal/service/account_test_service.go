...
type TestEvent struct {
	...
	// VideoURL is a data: or https URL for the in-browser media player.
...
// doOpenAIAccountTestUpstream keeps ordinary provider testing independent of
// the excluded plugin transport while preserving the existing TLS-fingerprint
// path used by supported OAuth/API-key accounts.
func (s *AccountTestService) doOpenAIAccountTestUpstream(request *http.Request, proxyURL string, account *Account, useTLSFallback bool) (*http.Response, error) {
	if useTLSFallback {
...
// then by selected model family for media. Image/video never hit Responses.
//
// Modes:
...
func (s *AccountTestService) testGrokAccountConnection(c *gin.Context, account *Account, modelID, prompt, mode string, opts AccountTestOptions) error {
	...
	// Explicit media modes always win over model id.
	switch mode {
	case AccountTestModeGrokImage:
