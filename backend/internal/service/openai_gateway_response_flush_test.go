...
func TestOpenAIResponseFlush_BurstRespectsSingleTokenBackpressureBoundaries(t *testing.T) {
	...
	require.LessOrEqual(t, len(flushes), len(burstEvents)+1)
	require.Equal(t, first, flushes[0])
	require.Equal(t, first+burst, flushes[len(flushes)-1])

	validBoundaries := map[string]bool{first: true}
	boundary := first
	for _, event := range burstEvents {
		boundary += event + "\n\n"
		validBoundaries[boundary] = true
	}
	previousLength := 0
	for _, flushed := range flushes {
		require.True(t, validBoundaries[flushed], "flush must occur after a complete SSE event")
		require.Greater(t, len(flushed), previousLength, "flush snapshots must advance monotonically")
		previousLength = len(flushed)
	}
}
...
func TestOpenAIResponseFlush_TerminalReadErrorFlushesResidual(t *testing.T) {
	body := "data: {\"type\":\"response.completed\",\"response\":{\"id\":\"resp_done\",\"status\":\"completed\",\"output\":[{\"type\":\"message\",\"role\":\"assistant\",\"content\":[{\"type\":\"output_text\",\"text\":\"done\"}]}]}}\n"
	recorder := newOpenAIResponseFlushRecorder()
	...
	require.Contains(t, gotBody, `"code":"`+OpenAIUpstreamStreamReadErrorCode+`"`)
...
func TestOpenAIPassthroughTerminalClosesJoinedScannerWithoutEOF(t *testing.T) {
	body := "data: {\"type\":\"response.completed\",\"response\":{\"status\":\"completed\",\"output\":[],\"usage\":{\"input_tokens\":7,\"output_tokens\":5}}}\n\n"
	reader := &hangingOpenAISSEAfterTerminal{payload: []byte(body), release: make(chan struct{})}
	t.Cleanup(func() { _ = reader.Close() })
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Request = httptest.NewRequest(http.MethodPost, "/v1/responses", nil)
	svc := &OpenAIGatewayService{cfg: &config.Config{Gateway: config.GatewayConfig{StreamDataIntervalTimeout: 30}}}
	type outcome struct {
	...
	go func() {
		result, err := svc.handleStreamingResponsePassthrough(context.Background(), &http.Response{StatusCode: http.StatusOK, Header: make(http.Header), Body: reader}, c, &Account{ID: 1, Platform: PlatformOpenAI}, time.Now(), "public", "upstream")
	...
	case out := <-done:
		...
		require.Equal(t, 7, out.result.usage.InputTokens)
		require.Equal(t, 5, out.result.usage.OutputTokens)
		select {
		case <-reader.release:
		default:
			t.Fatal("terminal return must close the body and release its reader")
		}
	case <-time.After(3 * time.Second):
		t.Fatal("passthrough waited for EOF after a complete provider terminal")
	}
}
