...
func startPassthroughLifecycleServer(
	...
	if len(hookFactories) > 0 {
		hooksFactory = hookFactories[0]
	}
	return startPassthroughLifecycleServerWithHooks(t, controlCtx, svc, account, hooksFactory)
}
...
func TestOpenAIWSPassthroughBillingModelRequiresExplicitAccountMapping(t *testing.T) {
	t.Parallel()

	account := &Account{Credentials: map[string]any{
		"model_mapping": map[string]any{
			"priced-channel-alias": "gpt-5.6-sol",
			"same-model":           "same-model",
		},
	}}

	require.Equal(t, "gpt-5.6-sol", openAIWSPassthroughBillingModel(account, "priced-channel-alias"))
	require.Empty(t, openAIWSPassthroughBillingModel(account, "unknown-channel-alias"),
		"unknown aliases must remain unpriced so local pricing fails closed")
	require.Empty(t, openAIWSPassthroughBillingModel(account, "same-model"))
	require.Empty(t, openAIWSPassthroughBillingModel(nil, "priced-channel-alias"))
}

func TestOpenAIWSPassthroughUnknownAliasFailsUsagePricingClosed(t *testing.T) {
	account := &Account{
		ID:       3003,
		Platform: PlatformOpenAI,
		Type:     AccountTypeAPIKey,
		Credentials: map[string]any{
			"model_mapping": map[string]any{"priced-channel-alias": "gpt-5.6-sol"},
		},
	}
	unknownAlias := "unknown-channel-alias"
	result := &OpenAIForwardResult{
		RequestID:     "ws_unknown_alias_pricing",
		Model:         "unknown-public-alias",
		BillingModel:  openAIWSPassthroughBillingModel(account, unknownAlias),
		UpstreamModel: unknownAlias,
	...
	require.ErrorContains(t, err, "calculate OpenAI usage cost failed")
