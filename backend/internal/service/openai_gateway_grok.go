...
func (s *OpenAIGatewayService) forwardGrokResponses(
	...
	if resp.StatusCode >= 400 {
		...
		handledResult.UpstreamFailed = true
		handledResult.UpstreamHTTPStatus = resp.StatusCode
		return finalizeOpenAIForwardResult(c, handledResult, patchedBody), handledErr
	}
	...
	if reqStream {
		...
		if streamResult == nil {
			...
			return finalizeOpenAIForwardResult(c, newGrokErrorForwardResult(resp, originalModel, upstreamModel, true, startTime), patchedBody), streamErr
		}
		...
		finishCapture()
		if err != nil {
			return finalizeOpenAIForwardResult(c, newGrokErrorForwardResult(resp, originalModel, upstreamModel, false, startTime), patchedBody), err
		}
...
func newGrokErrorForwardResult(resp *http.Response, originalModel, upstreamModel string, stream bool, startTime time.Time) *OpenAIForwardResult {
	...
	if resp != nil {
		result.UpstreamHTTPStatus = resp.StatusCode
