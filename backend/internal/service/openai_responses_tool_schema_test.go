...
// 总分配次数设置宽松绝对上限。AllocsPerRun 读取进程级计数，因此在隔离的 test
// 子进程中测量，避免完整 test suite 遗留的后台 goroutine 污染结果。
func TestSanitizeOpenAIResponsesToolParameterTypes_LargeHitSetDoesNotAllocatePerHit(t *testing.T) {
	const isolatedProbeEnv = "SUB2API_TEST_OPENAI_TOOL_SCHEMA_ALLOC_PROBE"
	if os.Getenv(isolatedProbeEnv) != "1" {
		testBinary, err := os.Executable()
		require.NoError(t, err)
		cmd := exec.Command(testBinary, "-test.run=^TestSanitizeOpenAIResponsesToolParameterTypes_LargeHitSetDoesNotAllocatePerHit$", "-test.count=1")
		cmd.Env = append(os.Environ(), isolatedProbeEnv+"=1")
		output, err := cmd.CombinedOutput()
		require.NoError(t, err, "isolated allocation probe failed:\n%s", output)
		return
	}
