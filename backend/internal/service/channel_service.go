...
func checkIntervalsHavePrices(p ChannelModelPricing) error {
	for _, iv := range p.Intervals {
		...
		if (p.BillingMode == BillingModePerRequest || p.BillingMode == BillingModeImage || p.BillingMode == BillingModeVideo) && iv.PerRequestPrice == nil {
			return infraerrors.BadRequest(
				"INTERVAL_MISSING_PER_REQUEST_PRICE",
				fmt.Sprintf("media billing interval requires per_request_price for model %v", p.Models),
			)
		}
	}
	return nil
