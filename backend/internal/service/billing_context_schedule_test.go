...
func TestResolveContextPricingSchedule_ParityWithBilling(t *testing.T) {
	...
	for _, sc := range scheduleScenarios() {
		...
		t.Run(sc.name, func(t *testing.T) {
			...
			if sc.platform != "" {
				ctx = context.WithValue(ctx, ctxkey.ForcePlatform, sc.platform)
			}
			...
			for _, c := range probes {
				...
				tier := tierAt(sched.Tiers, c)
				assertBucket := func(kind tokenKind, tokens UsageTokens, label string) {
					price := tierPricePtr(tier, kind)
					got, err := cost(tokens)
					if price == nil {
						require.ErrorIs(t, err, ErrModelPricingUnavailable, "%s@%d", label, c)
						return
					}
					require.NoError(t, err)
					assertCostClose(t, float64(c)**price, got, "%s@%d", label, c)
				}
				assertBucket(kindInput, UsageTokens{InputTokens: c}, "input")
				assertBucket(kindCacheRead, UsageTokens{CacheReadTokens: c}, "cache_read")
				assertBucket(kindCacheWrite, UsageTokens{CacheCreationTokens: c}, "cache_write")
				if tier.Output == nil {
					_, err := cost(UsageTokens{InputTokens: c, OutputTokens: outputProbe})
					require.ErrorIs(t, err, ErrModelPricingUnavailable, "output@%d", c)
				} else {
					withOutput, err := cost(UsageTokens{InputTokens: c, OutputTokens: outputProbe})
					require.NoError(t, err)
					inputOnly, err := cost(UsageTokens{InputTokens: c})
					require.NoError(t, err)
					assertCostClose(t, float64(outputProbe)**tier.Output, withOutput-inputOnly, "output@%d", c)
				}
			}
		})
	}
...
func tierPricePtr(tier ContextPricingTier, kind tokenKind) *float64 {
	switch kind {
	case kindInput:
		return tier.Input
	case kindOutput:
