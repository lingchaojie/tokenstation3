...
type groupCapacityKiroAccountRepoStub struct {
	mockAccountRepoForPlatform
	accounts []Account
}

func (s *groupCapacityKiroAccountRepoStub) ListSchedulableByGroupID(context.Context, int64) ([]Account, error) {
	return s.accounts, nil
}

func TestGroupCapacityRPMUsesSupportedAccountsOnly(t *testing.T) {
	svc := NewGroupCapacityService(
		&groupCapacityKiroAccountRepoStub{
			accounts: []Account{
				{
					ID:       1,
