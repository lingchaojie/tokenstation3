...
func TestUpstreamBillingProbeSuccessPersistsSanitizedSnapshot(t *testing.T) {
	...
	require.Nil(t, account.RateMultiplier, "a successful read-only probe must not change the account billing rate")
...
// Read-only observation accepts a zero declaration without changing billing.
func TestUpstreamBillingProbeObservationAcceptsZeroDeclaredRate(t *testing.T) {
	account := &Account{
		ID:          22,
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
