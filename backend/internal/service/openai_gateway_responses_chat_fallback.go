...
func (s *OpenAIGatewayService) forwardResponsesViaRawChatCompletions(
	...
	if clientStream {
		...
		result, forwardErr = s.streamChatCompletionsAsResponses(ctx, c, resp, originalModel, customTools, functionTools, toolSearch, namespaceTools, billingModel, upstreamModel, reasoningEffort, serviceTier, startTime)
	} else {
...
func (s *OpenAIGatewayService) streamChatCompletionsAsResponses(
	...
	scan := s.scanCCStream(ctx, c, resp, "openai responses chat fallback", requestID, startTime, func(chunk *apicompat.ChatCompletionsChunk) (bool, error) {
