...
type ContextPricingScheduleInput struct {
	...
	Group *Group
	// Platform 为请求的具体平台，决定渠道定价查找与平台旧规则的适用。
	Platform string
}
...
func (s *BillingService) ResolveContextPricingSchedule(ctx context.Context, resolver *ModelPricingResolver, in ContextPricingScheduleInput) (*ContextPricingSchedule, error) {
	...
	if in.Platform != "" {
		// Pricing lookup only needs the concrete platform selector. Keep this on
		// the existing ForcePlatform context contract; the excluded Composite
		// routing state must not leak back into billing.
	...
	if !resolved.HasResolvablePricing() {
		return nil, ErrModelPricingUnavailable
	}
