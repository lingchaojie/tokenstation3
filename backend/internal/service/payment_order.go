...
func (s *PaymentService) validateSubOrder(ctx context.Context, req CreateOrderRequest) (*dbent.SubscriptionPlan, error) {
	...
	if err := s.configService.ValidatePlanSeatAvailable(ctx, plan, req.UserID); err != nil {
		return nil, err
	}
	return plan, nil
}
...
func (s *PaymentService) buildPaymentSubject(plan *dbent.SubscriptionPlan, limitAmount float64, cfg *PaymentConfig, sel *payment.InstanceSelection) string {
	if plan != nil {
		...
		if productName == "" {
			productName = "LINX2.AI Subscription " + plan.Name
		}
	...
	return "LINX2.AI " + amountStr + " " + currency
}
