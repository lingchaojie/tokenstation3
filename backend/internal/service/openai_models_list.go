...
func projectAccountModelsBody(body []byte, account *Account, group *Group, codex bool) ([]byte, error) {
	...
	for _, id := range candidates {
		...
		if codex {
			if upstreamRaw, exists := byID[id]; exists {
				target, matched, raw, available = id, true, upstreamRaw, true
			} else if matched && !available && isOpenAICodexGPTModel(target) {
				encoded, err := json.Marshal(newConfiguredCodexModelDescriptor(target))
				if err != nil {
					return nil, err
				}
				raw, available = encoded, true
			}
		}
...
func (s *OpenAIGatewayService) FetchPinnedOpenAIModelsList(ctx context.Context, group *Group, maxAccountSwitches int, ifNoneMatch string) (*OpenAIModelsResponse, *Account, error) {
	...
	if group.CustomModelsListEnabled() {
		models = selectModelCatalogEntries(byID, group.ModelsListConfig.FilterForListing(modelIDs))
	}
...
func orderPinnedCodexModelsBySelection(body []byte, allowlist GroupModelsListConfig) ([]byte, error) {
