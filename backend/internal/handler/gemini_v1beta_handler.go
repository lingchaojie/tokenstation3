...
func (h *GatewayHandler) GeminiV1BetaListModels(c *gin.Context) {
	...
	filterGeminiModels := func(models []gemini.Model) []gemini.Model {
		...
		for _, model := range models {
			if apiKey.Group.ModelsListConfig.Allows(model.Name) {
...
// 调用方同样应透传原始响应。
func filterUpstreamGeminiModelsBody(body []byte, allowlist service.GroupModelsListConfig) (filtered []byte, dropped bool, ok bool) {
