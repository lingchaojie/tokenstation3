...
func newSimpleModeGroupRouter(svc *stubAdminService) *gin.Engine {
	...
	r.GET("/groups/:id/models-list-candidates", h.GetGroupModelsListConfigCandidates)
...
func TestGroupHandlerSimpleModeSanitizesCommercialFields(t *testing.T) {
	...
	create := `{"name":"simple","description":"basic grouping","platform":"anthropic","rate_multiplier":7,"is_exclusive":true,"subscription_type":"subscription","daily_limit_usd":10,"long_context_pricing_enabled":true,"model_pricing":[{"model":"claude","input_price":1}],"allow_image_generation":true,"allow_batch_image_generation":true,"video_price_720p":2,"web_search_price_per_call":3,"rpm_limit":99}`
...
func TestGroupHandlerSimpleModeResponseUsesFieldAllowlist(t *testing.T) {
	...
	svc.groups = []service.Group{{
		...
		WebSearchPricePerCall: float64PtrForSimpleModeTest(3),
