...
// GetGroupModelsListConfigCandidates handles getting candidate model IDs for the group model allowlist.
// GET /api/v1/admin/groups/:id/models-list-candidates
func (h *GroupHandler) GetGroupModelsListConfigCandidates(c *gin.Context) {
...
func (h *GroupHandler) Delete(c *gin.Context) {
	...
	if h.isSimpleMode() {
		deleter, ok := h.adminService.(interface {
			DeleteGroupIfEmpty(context.Context, int64) error
		})
		if !ok {
			err = fmt.Errorf("guarded group deletion is unavailable")
		} else {
			err = deleter.DeleteGroupIfEmpty(c.Request.Context(), groupID)
		}
	} else {
