...
func (h *SettingHandler) buildSystemSettingsPayload(
	settings *service.SystemSettings,
	paymentCfg *service.PaymentConfig,
	defaultSubscriptions []dto.DefaultSubscriptionSetting,
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	settings := &service.SystemSettings{
		...
		OpenAITTFTMode:                         stringSetting(req.OpenAITTFTMode, previousSettings.OpenAITTFTMode),
	...
	payload := h.buildSystemSettingsPayload(updatedSettings, updatedPaymentCfg, updatedDefaultSubscriptions, updatedOpsEnabled && updatedSettings.OpsMonitoringEnabled)
	if fastPolicy, err := h.settingService.GetOpenAIFastPolicySettings(c.Request.Context()); err != nil {
		slog.Error("openai_fast_policy_settings_get_failed", "error", err)
	} else if fastPolicy != nil {
		payload.OpenAIFastPolicySettings = openaiFastPolicySettingsToDTO(fastPolicy)
...
// Task 8 semantic union: declarations retained from the pinned upstream settings split.
