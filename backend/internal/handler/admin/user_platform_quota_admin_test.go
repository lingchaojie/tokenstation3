...
func TestUpdateUserPlatformQuotas_Success(t *testing.T) {
	...
	expectedDeletes := len(service.AllowedQuotaPlatforms)
	if len(cache.deleteCalls) != expectedDeletes {
		t.Errorf("expected %d cache delete calls, got %d: %+v", expectedDeletes, len(cache.deleteCalls), cache.deleteCalls)
	}
}
