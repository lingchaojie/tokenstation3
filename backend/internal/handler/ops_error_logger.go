...
func OpsErrorLoggerMiddleware(ops *service.OpsService) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		w.ctx = c
		defer func() {
			// Downstream handlers may wrap w (for example compact SSE keepalive)
			// and leave that wrapper in c.Writer. Always sever the entire wrapper
			// chain before returning w to the pool; otherwise an outer middleware
			// can dereference a released or already-reused capture writer.
		...
		if status < 400 {
			// A durable logical failure discovered after the response was
			// committed is the terminal outcome of this request. Prefer it over
			// recovered upstream-attempt diagnostics, which remain useful only
			// when the final request itself succeeded.
...
func classifyOpsErrorLog(c *gin.Context, errType, message, code string, status int) (phase string, isBusinessLimited bool, errorOwner string, errorSource string) {
	...
	if strings.TrimSpace(code) == usagePricingUnavailableCode {
		phase = "internal"
		return phase, false, classifyOpsErrorOwner(phase, message), classifyOpsErrorSource(phase, message)
	}
