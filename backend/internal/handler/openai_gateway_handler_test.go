...
func TestOpenAIWSTurnBillingModelPreservesImagePricingModel(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			require.Equal(t, tt.wantBillingModel, openAIWSTurnBillingModel(result, tt.mapping, tt.requestedModel, tt.upstreamModel, nil))
		})
	}
}

func TestOpenAIWSTurnBillingModelPreservesExplicitAccountAliasMapping(t *testing.T) {
	...
	mapping := service.ChannelMappingResult{
		MappedModel:        "priced-channel-alias",
		BillingModelSource: service.BillingModelSourceChannelMapped,
	}

	require.Equal(t, "gpt-5.6-sol", openAIWSTurnBillingModel(
		&service.OpenAIForwardResult{BillingModel: "gpt-5.6-sol"},
		mapping,
		"public-alias",
		"priced-channel-alias",
		account,
	))
	require.Equal(t, "unknown-channel-alias", openAIWSTurnBillingModel(
		&service.OpenAIForwardResult{},
		service.ChannelMappingResult{
			MappedModel:        "unknown-channel-alias",
			BillingModelSource: service.BillingModelSourceChannelMapped,
		},
...
func runOpenAIResponsesWebSocketUsageLogCase(t *testing.T, tc openAIResponsesWSUsageLogCase) openAIResponsesWSUsageLogResult {
	...
	if tc.captureSetup != nil {
		settings, capturePool, captureRecords = tc.captureSetup(t, cfg)
	}
