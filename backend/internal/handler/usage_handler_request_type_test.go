...
func TestUserUsageListKeepsUserBillingAndIPWithoutAdminCostFields(t *testing.T) {
	...
	// DEV masking: per-field costs are scaled to actual cost, total_cost==actual_cost
	// and rate_multiplier is normalized to 1 so the markup is hidden from users.
	// Exact per-field scaling is covered by dto.TestUsageLogFromService_NormalizesBillingForRegularUsers.
...
func TestUserUsageStatsUsesScopedFilters(t *testing.T) {
	...
	// DEV masking: regular users see actual cost as total_cost (markup hidden).
...
func TestUserUsageDashboardModelsOmitsAccountCost(t *testing.T) {
	...
	require.Equal(t, http.StatusOK, rec.Code)
	body := rec.Body.String()
	// DEV masking: regular users see actual cost as cost (markup hidden).
