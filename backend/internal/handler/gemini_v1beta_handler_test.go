...
func TestGeminiV1BetaListModels_AllowlistFiltersNativeResponse(t *testing.T) {
	...
	filtered, dropped, ok := filterUpstreamGeminiModelsBody(body, service.GroupModelsListConfig{Enabled: true, Models: []string{"gemini-2.5-pro"}})
	require.True(t, ok)
...
func TestGeminiV1BetaListModels_ForcedAntigravityAppliesAllowlist(t *testing.T) {
	...
	key.GroupID = &groupID
	repo := &geminiModelsAccountRepoStub{}
...
func TestGeminiModelsListConfig_DisabledPreservesNativeResponse(t *testing.T) {
	body := []byte(`{"models":[{"name":"models/gemini-2.5-pro"}]}`)
	filtered, dropped, ok := filterUpstreamGeminiModelsBody(body, service.GroupModelsListConfig{Enabled: false, Models: []string{"other"}})
	require.True(t, ok)
