...
func (h *BatchImageHandler) Models(c *gin.Context) {
	...
	if apiKey, ok := middleware.GetAPIKeyFromContext(c); ok && apiKey != nil && apiKey.Group != nil {
		got.Data = filterBatchImageModelsByAllowlist(got.Data, apiKey.Group.ModelsListConfig)
	}
	c.JSON(http.StatusOK, got)
}
...
// 白名单未开启时原样返回。
func filterBatchImageModelsByAllowlist(models []service.BatchImagePublicModel, allowlist service.GroupModelsListConfig) []service.BatchImagePublicModel {
