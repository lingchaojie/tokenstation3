...
func TestSettingHandler_GetPublicSettings_OmitsDeferredAuthenticationRolloutFields(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	h := NewSettingHandler(service.NewSettingService(repo, &config.Config{WebAuthn: config.WebAuthnConfig{
		Enabled:   true,
		RPID:      "example.test",
	...
	for _, field := range []string{
		...
		_, exposed := resp.Data[field]
		require.False(t, exposed, "deferred authentication field must not be public: %s", field)
	}
}
...
func TestSettingHandler_GetPublicModelCatalog_ReturnsCompleteCatalog(t *testing.T) {
	...
	require.NoError(t, json.Unmarshal(recorder.Body.Bytes(), &resp))
	require.Equal(t, 0, resp.Code)
	require.Equal(t, "2026-09-05", resp.Data.UpdatedAt)
	require.Len(t, resp.Data.Models, 39)
