...
func (h *GatewayHandler) Messages(c *gin.Context) {
	...
	service.SetCaptureSessionID(c, parsedReq, sessionHash)
	...
	// 获取平台：优先使用强制平台（/antigravity 路由），否则使用分组平台
	platform := ""
	...
	if platform == service.PlatformGemini {
		...
		for {
			...
			if selection.ProfitGateActive() || !selection.Acquired {
				if err := h.gatewayService.BindStickySessionForAccountGroupAfterProfitAdmission(admissionCtx, apiKey.GroupID, sessionKey, account, apiKey.Group); err != nil {
			...
			if account.SupportsAccountRPM() && account.GetBaseRPM() > 0 {
	...
	for {
		...
		for {
			...
			// 抢槽的直达路径无门时由选号内部绑定，这里只在门下补准入后绑定。
			if selection.ProfitGateActive() || !selection.Acquired {
				if err := h.gatewayService.BindStickySessionForAccountGroupAfterProfitAdmission(admissionCtx, currentAPIKey.GroupID, sessionKey, account, currentAPIKey.Group); err != nil {
					reqLog.Warn("gateway.bind_sticky_session_after_profit_admission_failed", zap.Int64("account_id", account.ID), zap.Error(err))
				}
			}
			// 账号槽位/等待计数需要在超时或断开时安全回收
			...
			forwardSideEffects := newGatewayForwardSideEffectSubmitterWithEffects(
			...
			if err != nil {
				...
				if result != nil && !result.UpstreamFailed && result.HasBillableUsage() {
					// 上游已计量的部分响应保留会话名额；本地 capture-only
					// 失败结果不代表会话已被服务，仍按上游失败规则释放。
...
func (h *GatewayHandler) Models(c *gin.Context) {
	...
	if apiKey != nil && apiKey.Group != nil && apiKey.Group.CustomModelsListEnabled() {
		...
		writeAllowlistedModelsList(c, platform, filterModelsByCustomList(source, fallbackModels, apiKey.Group.ModelsListConfig.Models))
		return
	}
...
func (h *GatewayHandler) AntigravityModels(c *gin.Context) {
	...
	if apiKey, ok := middleware2.GetAPIKeyFromContext(c); ok && apiKey != nil && apiKey.Group != nil && apiKey.Group.CustomModelsListEnabled() {
		...
		for _, model := range models {
			if apiKey.Group.ModelsListConfig.Allows(model.ID) {
				filtered = append(filtered, model)
			}
		}
		models = filtered
