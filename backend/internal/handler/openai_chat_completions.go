	...
	for {
		...
		submitChatUsage := func(res *service.OpenAIForwardResult) {
			...
			if res.UpstreamFailed {
				h.submitCapture(c, res, account, body, upstreamEndpoint)
				return
			}
		...
		if err != nil {
			if result != nil && result.UpstreamFailed {
				h.gatewayService.ReportOpenAIAccountScheduleResult(account, openAIAccountScheduleModel(c, account, reqModel, false, result), false, result.FirstTokenMs, err)
		...
		if result != nil {
			h.gatewayService.ReportOpenAIAccountScheduleResult(account, openAIAccountScheduleModel(c, account, reqModel, false, result), openAIForwardSucceededForScheduling(result), result.FirstTokenMs)
		} else {
