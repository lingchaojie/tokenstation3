...
func (h *UsageHandler) parseUserUsageFilters(c *gin.Context, requireRange bool) (*userUsageFilters, bool) {
	...
	return &userUsageFilters{
		Filters: usagestats.UsageLogFilters{
			...
			RequestedReasoningEffort: strings.TrimSpace(c.Query("reasoning_effort")),
...
func usageStatsForRegularUser(stats *usagestats.UsageStats) *usagestats.UsageStats {
	if stats == nil {
		return nil
	}
	out := *stats
...
func userModelStatsFromUsageStats(stats []usagestats.ModelStat) []userModelStat {
	...
	for _, stat := range stats {
		out = append(out, userModelStat{
			...
			// DEV: hide backend markup from users — expose actual cost only.
