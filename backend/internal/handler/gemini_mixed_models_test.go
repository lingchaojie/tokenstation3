...
type geminiModelsAccountRepoStub struct {
	gatewayModelsAccountRepoStub
}

func (s *geminiModelsAccountRepoStub) ListSchedulableByGroupIDAndPlatforms(_ context.Context, groupID int64, platforms []string) ([]service.Account, error) {
...
func TestGeminiNativeModelsUsesAccountMappings(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			groupID := int64(45)
			repo := &geminiModelsAccountRepoStub{gatewayModelsAccountRepoStub: gatewayModelsAccountRepoStub{byGroup: map[int64][]service.Account{
			...
			c.Set(string(middleware.ContextKeyAPIKey), &service.APIKey{GroupID: &groupID, Group: &service.Group{ID: groupID, Platform: service.PlatformGemini,
				ModelsListConfig: service.GroupModelsListConfig{Enabled: tt.allowlist, Models: []string{"gemini-synced-custom"}},
			}})
			if tt.forced {
...
func TestAppendUpstreamGeminiModelsPreservesMetadata(t *testing.T) {
	...
	filtered, dropped, ok := filterUpstreamGeminiModelsBody(merged, service.GroupModelsListConfig{Enabled: true, Models: []string{"gemini-synced-*"}})
	require.True(t, ok)
...
func TestGeminiNativeModelsMergesNativeUpstream(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			id := int64(46)
			repo := &geminiModelsAccountRepoStub{gatewayModelsAccountRepoStub: gatewayModelsAccountRepoStub{byGroup: map[int64][]service.Account{id: {
