...
func TestCodexModelsPinnedOverridesLocalSourceButKeepsAliasesAndFiltering(t *testing.T) {
	for _, enabled := range []bool{false, true} {
		t.Run(fmt.Sprintf("enabled=%t", enabled), func(t *testing.T) {
			account := newPinnedCodexAccount(2, service.StatusActive, true, false)
			account.Credentials["model_mapping"] = map[string]any{"local-alias": "gpt-5.6-sol"}
			upstream := &codexModelsPinnedHTTPUpstream{bodies: map[int64]string{
				2: `{"models":[{"slug":"pinned-only"},{"slug":"hidden-model"}]}`,
			}}
			handler := newPinnedCodexTestHandler([]service.Account{account}, upstream, 3)
			group := &service.Group{
				ID: 86, Platform: service.PlatformOpenAI,
				CodexModelsManifestConfig: service.GroupCodexModelsManifestConfig{
					Enabled: enabled, AccountIDs: []int64{2},
				},
				ModelsListConfig: service.GroupModelsListConfig{
					Enabled: true, Models: []string{"local-alias", "pinned-only"},
				},
			}
			first := performPinnedCodexModelsRequest(t, handler, group, "")
			require.Equal(t, http.StatusOK, first.Code, first.Body.String())
			if enabled {
				require.ElementsMatch(t, []string{"local-alias", "pinned-only"}, codexHandlerManifestSlugs(t, first))
				require.Equal(t, []int64{2}, upstream.accountIDs())
			} else {
				require.Equal(t, []string{"local-alias"}, codexHandlerManifestSlugs(t, first))
				require.Empty(t, upstream.accountIDs())
			}
			etag := first.Header().Get("ETag")
			require.NotEmpty(t, etag)
			group.ModelsListConfig.Models = []string{"local-alias"}
			filtered := performPinnedCodexModelsRequest(t, handler, group, etag)
			if enabled {
				require.Equal(t, http.StatusOK, filtered.Code, filtered.Body.String())
				require.Equal(t, []string{"local-alias"}, codexHandlerManifestSlugs(t, filtered))
				require.NotEqual(t, etag, filtered.Header().Get("ETag"))
				etag = filtered.Header().Get("ETag")
			} else {
				require.Equal(t, http.StatusNotModified, filtered.Code)
			}
			unchanged := performPinnedCodexModelsRequest(t, handler, group, etag)
