...
func TestAttemptSplitsPayloadsAndSendsTerminalSequence(t *testing.T) {
	...
	require.Equal(t, KindResponseChunk, frames[2].header.Kind)
	require.Len(t, frames[2].payload, MaxPayloadBytes)
	require.Equal(t, KindResponseChunk, frames[3].header.Kind)
	require.Equal(t, body[MaxPayloadBytes:], frames[3].payload)
	require.Equal(t, KindFinal, frames[4].header.Kind)
	require.Equal(t, KindCommit, frames[5].header.Kind)
