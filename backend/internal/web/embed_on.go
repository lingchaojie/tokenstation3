...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	if err != nil {
		// Fallback: serve without a stale injected config or stale ETag.
		s.serveBaseIndexHTML(c, nonce)
		return
	}
	...
	cached := s.cache.Get(settingsHash)
	if cached == nil {
		rendered := s.injectSettings(settingsJSON)
		cached = s.cache.Set(rendered, settingsJSON)
	}

	// Check If-None-Match only after confirming the cache matches current settings.
...
func (s *FrontendServer) serveBaseIndexHTML(c *gin.Context, nonce string) {
...
func injectSiteTitle(html, settingsJSON []byte) []byte {
	...
	newTitle := []byte("<title>" + htmlpkg.EscapeString(cfg.SiteName) + " - AI Gateway Platform</title>")
