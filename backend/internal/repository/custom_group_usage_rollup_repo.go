...
func (r *usageLogRepository) getAllGroupUsageSummaryFromRollups(ctx context.Context, todayStart time.Time) (results []usagestats.GroupUsageSummary, err error) {
	if db, ok := r.sql.(*sql.DB); ok {
		return withGroupUsageRollupReadSnapshot(ctx, db, func(tx *sql.Tx) ([]usagestats.GroupUsageSummary, error) {
			return (&usageLogRepository{sql: tx}).getAllGroupUsageSummaryFromRollups(ctx, todayStart)
		})
	}
	...
	return r.queryGroupUsageRollupSnapshot(ctx, todayStart, state)
}

// The watermark and totals used to share one SQL statement. Keep that snapshot
// guarantee while retaining parameterized tail bounds for the index scan.
func withGroupUsageRollupReadSnapshot(ctx context.Context, db interface {
	BeginTx(context.Context, *sql.TxOptions) (*sql.Tx, error)
}, read func(*sql.Tx) ([]usagestats.GroupUsageSummary, error)) ([]usagestats.GroupUsageSummary, error) {
...
func (r *usageLogRepository) queryGroupUsageRollupSnapshot(ctx context.Context, todayStart time.Time, state groupUsageRollupSnapshot) (results []usagestats.GroupUsageSummary, err error) {
