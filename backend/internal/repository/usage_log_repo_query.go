...
func (r *usageLogRepository) ListWithFilters(ctx context.Context, params pagination.PaginationParams, filters UsageLogFilters) ([]service.UsageLog, *pagination.PaginationResult, error) {
	...
	conditions, args = appendUsageObservabilityWhereConditions(conditions, args, filters, "")
...
func appendUsageObservabilityWhereConditions(conditions []string, args []any, filters UsageLogFilters, alias string) ([]string, []any) {
	prefix := ""
	if alias != "" {
		prefix = alias + "."
	}
	appendExact := func(column, value string) {
		if value = strings.TrimSpace(value); value != "" {
			conditions = append(conditions, fmt.Sprintf("%s%s = $%d", prefix, column, len(args)+1))
			args = append(args, value)
		}
	}
	appendExact("service_tier", filters.ServiceTier)
	appendExact("reasoning_effort", filters.ReasoningEffort)
	if value := strings.TrimSpace(filters.RequestedReasoningEffort); value != "" {
		conditions = append(conditions, fmt.Sprintf("COALESCE(NULLIF(TRIM(%srequested_reasoning_effort), ''), %sreasoning_effort) = $%d", prefix, prefix, len(args)+1))
		args = append(args, value)
	}
	appendExact("inbound_endpoint", filters.InboundEndpoint)
	appendExact("upstream_endpoint", filters.UpstreamEndpoint)
	return conditions, args
}

func appendUsageObservabilityQueryFilters(query string, args []any, filters UsageLogFilters, alias string) (string, []any) {
	conditions, args := appendUsageObservabilityWhereConditions(nil, args, filters, alias)
