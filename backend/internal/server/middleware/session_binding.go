...
// SessionBindingContext 全局中间件：始终快照客户端 IP 解析设置；仅在会话绑定
// 明确启用时将客户端 IP 与 User-Agent 注入 request context，供 token 签发路径
// （登录 / 刷新 / OAuth 回调）与会话绑定校验读取。
// IP 取值与 API Key IP 限制共用转发 IP 开关：开启时旧版原始转发头逻辑
// 接管解析，关闭时使用 Gin 的 server.trusted_proxies 可信代理链。
func SessionBindingContext(cfg *config.Config, settingService *service.SettingService) gin.HandlerFunc {
