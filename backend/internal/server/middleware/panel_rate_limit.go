...
// PanelRateLimiter is a dormant backend foundation. No current router constructs
// or mounts it; the methods below are retained for a separately approved rollout.
//
// 设计要点：
...
// Global builds the prospective authenticated per-user limiter.
...
// Heavy builds the prospective per-user limiter for expensive endpoints.
...
// PublicIP builds the prospective unauthenticated per-IP limiter.
