...
func TestOpenAICompatibleImagesEndToEnd(t *testing.T) {
	...
	for _, scenario := range []string{"generation", "json_edit", "multipart_alias", "disabled", "native_only", "restricted"} {
		t.Run(scenario, func(t *testing.T) {
			...
			groupID := int64(101)
			price := 0.17
			group := &service.Group{ID: groupID, Platform: service.PlatformOpenAI, AllowImageGeneration: scenario != "disabled", RateMultiplier: 1, ImagePrice1K: &price, ImagePrice2K: &price, ImagePrice4K: &price}
			accounts := []service.Account{}
			...
			for i, typ := range []string{service.AccountTypeOAuth, service.AccountTypeSetupToken, service.AccountTypeAPIKey} {
				...
				if scenario == "multipart_alias" {
					mapping["gemini-public-image"] = model
				}
			...
			imagesHandler := handler.NewOpenAIGatewayHandler(gateway, service.NewConcurrencyService(nil), billingCache, service.NewAPIKeyService(nil, nil, nil, nil, nil, nil, cfg), nil, nil, nil, nil, cfg, nil)
			publicModel := model
			if scenario == "multipart_alias" {
				publicModel = "gemini-public-image"
			}
			router := gin.New()
