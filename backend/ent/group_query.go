...
// QueryAPIKeyRoutes chains the current query on the "api_key_routes" edge.
func (_q *GroupQuery) QueryAPIKeyRoutes() *UserAPIKeyRouteQuery {
	query := (&UserAPIKeyRouteClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// WithAPIKeyRoutes tells the query-builder to eager-load the nodes that are connected to
// the "api_key_routes" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *GroupQuery) WithAPIKeyRoutes(opts ...func(*UserAPIKeyRouteQuery)) *GroupQuery {
	query := (&UserAPIKeyRouteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withAPIKeyRoutes = query
	return _q
}
...
func (_q *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := _q.withAPIKeyRoutes; query != nil {
		if err := _q.loadAPIKeyRoutes(ctx, query, nodes,
			func(n *Group) { n.Edges.APIKeyRoutes = []*UserAPIKeyRoute{} },
			func(n *Group, e *UserAPIKeyRoute) { n.Edges.APIKeyRoutes = append(n.Edges.APIKeyRoutes, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *GroupQuery) loadAPIKeyRoutes(ctx context.Context, query *UserAPIKeyRouteQuery, nodes []*Group, init func(*Group), assign func(*Group, *UserAPIKeyRoute)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Group)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userapikeyroute.FieldGroupID)
	}
	query.Where(predicate.UserAPIKeyRoute(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.APIKeyRoutesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
