...
func (_q *UserQuery) QueryAPIKeyRoutes() *UserAPIKeyRouteQuery {
...
func (_q *UserQuery) WithAPIKeyRoutes(opts ...func(*UserAPIKeyRouteQuery)) *UserQuery {
	query := (&UserAPIKeyRouteClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) loadAPIKeyRoutes(ctx context.Context, query *UserAPIKeyRouteQuery, nodes []*User, init func(*User), assign func(*User, *UserAPIKeyRoute)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(userapikeyroute.FieldUserID)
	}
