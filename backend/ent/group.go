...
type Group struct {
	...
	// Anthropic/OpenAI reasoning effort 自定义映射；可按模型精确名、前缀或后缀限定，先映射再应用上限
...
// was not loaded in eager-loading.
func (e GroupEdges) APIKeyRoutesOrErr() ([]*UserAPIKeyRoute, error) {
	if e.loadedTypes[4] {
...
// QueryAPIKeyRoutes queries the "api_key_routes" edge of the Group entity.
func (_m *Group) QueryAPIKeyRoutes() *UserAPIKeyRouteQuery {
	return NewGroupClient(_m.config).QueryAPIKeyRoutes(_m)
}
