...
- `TestAdminServiceBulkUpdateAccounts_RejectsEveryInvalidCursorActivationBeforeWrite` — `backend/internal/service/admin_service_bulk_update_test.go`; covers explicit-ID and filter-resolved bulk targets for both active-status and schedulable-enable operations, rejecting every invalid Cursor type before repository mutation or group binding. Companion bulk cases preserve atomic rejection for mixed targets while still allowing quarantine operations.
