...
if ! printf '%s\n' "$active_config" | grep -Eq '^[[:space:]]*reverse_proxy[[:space:]]+(localhost|127\.0\.0\.1):8080'; then
	echo "Caddyfile must continue proxying all application routes to the local service on port 8080" >&2
	exit 1
fi
